...
-   **📋 Tier naming.** The Stripe price envs are `STRIPE_PRICE_CANOPY_*`
    (`services/subscriptions.ts` `CANOPY_PRODUCTS`: free/sprout/canopy_pro) while
    the FBM catalog tiers are Signal/Coalition/Sovereign. These are **two distinct
    systems** — the native platform subscription (Canopy, Stripe-billed) vs. the
    FBM `subscription_tier` listings (feature-bundle grants). Not a defect, but the
    overlap is a documentation hazard; keep them clearly separated.
...
| unique_inventory         | 🔧 stub    | —                 | quantity-lock-to-1 / no-relist unenforced. Add the `quantity !== 1` guard (pattern: `validate-ticket-order.ts:32`) at add-to-cart/create + decrement-to-zero on `order.placed` (pattern: `sync-lot-inventory.ts:109`) + a status flip blocking relist.                                                                                                                                                                                                                                                         |
