...
| 4 | Synapse homeserver | All Blackout-side communication, federation, governance ACL sync, entitlements consumption | Capacity telemetry; media retention policy; worker-mode config staged | Foundation (telemetry); Density (workers or Dendrite/conduwuit if I/O is binding) | [`../../infra/single-server-baseline/RUNBOOK.md`](../../infra/single-server-baseline/RUNBOOK.md) |
...
| 7 | Secrets manager (Vault / Infisical / SOPS) | Every credentialed integration | Distributed secrets today; consolidation Foundation work | Foundation | [`../runbooks/SECRETS_MANAGER_MIGRATION.md`](../runbooks/SECRETS_MANAGER_MIGRATION.md), [`secrets_rotation_break_glass.md`](secrets_rotation_break_glass.md) |
