...
| Rank | Item | Risk class | Owner | Milestone |
| --- | --- | --- | --- | --- |
| 1 | `src/components/views/messages/MImageBody.tsx` image-size cap behavior | Data/UI correctness | Timeline UX | 2026.04 |
| 2 | `src/components/views/room_settings/AliasSettings.tsx` alias validation/error surfacing | Security + data correctness | Rooms/Identity | 2026.04 |
| 3 | `src/indexing/EventIndex.ts` lazy-loading assumptions in indexing | Data correctness/perf | Search/Indexing | 2026.04 |
| 4 | `src/stores/spaces/SpaceStore.ts` parent-rebuild permission handling | Data correctness | Spaces | 2026.04 |
| 5 | `src/components/views/right_panel/VerificationPanel.tsx` QR camera entry path | Security UX | Security UX | 2026.05 |
| 6 | `src/components/views/rooms/LinkPreviewWidget.tsx` media rendering factoring | Reliability/UX | Media UX | 2026.05 |
| 7 | `src/components/views/rooms/Stickerpicker.tsx` multi-store support | Product capability | Integrations | 2026.05 |
| 8 | `src/components/views/settings/Notifications.tsx` view logic decomposition | Maintainability | Notifications | 2026.05 |
| 9 | `src/stores/widgets/WidgetStore.ts` broader widget-store consolidation | Maintainability | Widgets Platform | 2026.06 |
| 10 | `src/autocomplete/UserProvider.tsx` sender-member lazy-load fallback | Perf + correctness | Composer/Autocomplete | 2026.06 |
...
- Commands:
  ...
  - `rg -n "Open items: \*\*54\*\*|open marker inventory: 54|backlog remains high \(54\)" docs/unfinished-code-checklist.md docs/project_completion_tracker.md docs/blackout_centralized_release_readiness_gate.md`
