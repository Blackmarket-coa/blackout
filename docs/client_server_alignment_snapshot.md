...
1. **Townhall backend owner is `Blackout_server` and should be treated as the canonical implementation target.**
    - Client integration expects `POST /api/townhall/token` and role-policy-compatible responses from the backend.
    - Backend implementation and rollout should land in `https://github.com/Blackmarket-coa/Blackout_server`, with this repo tracking client-side contract expectations and integration evidence.
...
1. Publish/maintain a **shared contract doc** for townhall token request/response schema and auth semantics, owned with `Blackout_server`.
2. Add a **cross-repo integration test**:
    - client `TownhallTokenService` request shape ↔ `Blackout_server` token endpoint response contract.
3. Pin both repos to a **compatibility matrix** (`client commit` ↔ `Blackout_server` commit) in release docs.
