...
- **Status (2026-04-27): foundation landed; UI rewire pending.**
  ...
  - **2026-04-30 production fetcher wiring:** every BKL page now resolves its fetcher via `props ?? useRegistryFetcher(key) ?? stub` (preserving the explicit-prop test pattern). New `apps/blackout-client/src/app/core/features/registryFetchers.ts` exposes `buildRegistryFetchers(apiClient)` that produces a `RegistryFetchers` bag covering all 13 page fetcher types; `apps/blackout-client/src/app/core/features/RegistryFetcherProvider.tsx` exposes the provider + `useRegistryFetcher(key)` hook. `main.tsx` builds an `ApiClient` via `createFetchApiClient({ baseUrl: import.meta.env.VITE_BLACKOUT_API_BASE })` once at boot and mounts `<RegistryFetcherProvider fetchers={registryFetchers}>` around the router. Page tests are unaffected — they still pass an explicit `fetcher` prop, which wins over the context. 7 new fetcher tests cover the bag wiring (every page fetcher hits its canonical SDK path), the auth envelope-unwrap shim, the provider's default-null behavior, the partial-provider miss path, and the explicit-prop precedence rule.
  - Remaining acceptance work: actually mount `RegistrySidebarList` / `RegistrySettingsList` in the modern `ClientLayout` / `Settings.tsx` shells (the rewire layer + production fetcher wiring are now in place; surface-by-surface adoption is mechanical).
...
- **Status (2026-04-30): foundation + finished UI landed.**
  - Protocol: `packages/blackout-protocol/src/federated-ops/{contracts,events}.ts` publishes `FederationAlertStatusPayload` (with `FederationAlertSeverity` = `info|warning|critical`), `TownhallLifecyclePayload` (with `TownhallLifecyclePhase` = `scheduled|live|archived|cancelled`), `RevenueOpsSnapshotPayload` (precision-safe string figures matching the BKL-003 treasury convention), the matching envelope types, and `isFederationAlertStatus` / `isTownhallLifecycle` / `isRevenueOpsSnapshot` type guards. `BlackoutEventName` is extended with all three new event names; `FEDERATED_OPS_EVENT_NAMES` covers the `co.bmc.federation.alert.status` / `co.bmc.townhall.lifecycle` / `co.bmc.revenue.ops.snapshot` Matrix event types.
  - SDK: `packages/blackout-sdk/src/federated-ops/actions.ts` ships `createFederatedOpsActions(client)` with federation alerts (`listAlerts`, `acknowledgeAlert`), townhall directory + transitions (`listTownhalls`, `transitionTownhall` — strips `cancellationReason` on non-cancelled phases), and revenue ops (`getRevenueSnapshot`, `listRevenueSnapshots` with cursor + limit pagination, `publishRevenueSnapshot`). Adds two pure helpers: `compareFederationSeverity` (orders `critical → warning → info`, unknown values sink) and `applyTownhallLifecycle` (merges a lifecycle envelope into a snapshot, returning the same reference on townhallId mismatch).
  - Canonical client: new `apps/blackout-client/src/app/features/federated-ops/` module with three capability-gated customizations — `federation-health` (`federation.health.read`), `townhall-ops` (`townhall.ops.manage`), `revenue-ops` (`revenue.ops.read`) — all behind a new `federatedOps` flag (default off, `BLACKOUT_FEDERATED_OPS` override on every feature mode). Each customization contributes route + workspace + sidebar entry + settings section. Real renderers shipped: `FederationHealthPage` (severity-sorted alert list with optimistic acknowledge), `TownhallPage` (phase transition with conditional cancellation reason), `RevenueOpsPage` (latest snapshot + history with separate error regions). All accept injectable fetchers.
  - Tests: `apps/blackout-client/tests/unit/sdk/federatedOpsActions.test.ts` (12 cases) covers event-type strings, type-guard narrowing for all three envelopes (severity / phase / figures shape), every SDK action's request shape (URL encoding + `cancellationReason` strip + cursor/limit clamp), and the two pure helpers. `apps/blackout-client/tests/unit/core/features/federatedOpsModule.test.ts` (4 cases) covers per-capability surface exposure + flag pruning. `apps/blackout-client/tests/unit/features/federated-ops/FederatedOpsPages.test.tsx` (8 cases) covers severity ordering, resolved-alert hide, optimistic acknowledge, townhall empty/transition flows, conditional reason input, and revenue page render + error.
