...
- `apps/blackout-client` entry currently initializes web app + service worker/session sync, but does not provide equivalent native bridge integration points consumed by wrappers.【F:apps/blackout-client/src/index.tsx†L1-L56】

---

## Regressions / unresolved parity gaps

| Gap ID | Area | Severity | Regression risk if wrappers switch to canonical now | Owner | Remediation ETA |
|---|---|---|---|---|---|
| WRAP-001 | Deep-link bridge parity (`deep_link_opened`) | High | Wrapper deep links will not route through expected canonical bridge path. | Frontend Platform Team | 2026-04-29 |
| WRAP-002 | Notification bridge parity (`notification_token`, `notification_interacted`, unread sync) | High | Push token registration, notification click routing, and unread tray sync may break/regress. | Frontend Notifications Team | 2026-05-03 |
| WRAP-003 | Lifecycle parity (`resume_sync`, app foreground/background + desktop visibility lifecycle) | Medium | Resume-sync and lifecycle-driven refresh behavior may diverge. | Frontend Platform Team | 2026-05-06 |
| WRAP-004 | Native share/camera/media bridge parity | Medium | Mobile native share/camera pathways may fall back or become inconsistent. | Frontend Media Team | 2026-05-10 |
