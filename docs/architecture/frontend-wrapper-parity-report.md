...
| Share / camera / media bridges | **Partial pass**: native share helper exists; camera/plugin permissions configured in Capacitor. | **N/A/Partial**: desktop uses native window + notification shell but no explicit share/camera bridge layer. | **Pass for share** (WRAP-004 closed 2026-04-27): canonical client exposes `nativeShare(payload)` and `nativeCanShare()` in `apps/blackout-client/src/platform/nativeMediaBridge.ts`, dispatching to `@capacitor/share` (mobile native sheet) when available, then `navigator.share` (Web Share API in modern browsers and Tauri webview), then a `navigator.clipboard.writeText` fallback for the canonical url/text. **Camera/media-pick** is gated by composer-attachment ports tracked in BKL-006 and is therefore deferred to that backlog item. | **Pass** |
...
| WRAP-004 | Native share/camera/media bridge parity | Medium | **Closed 2026-04-27 (share)** | Resolved for share: `nativeShare()` delegates to `@capacitor/share` → `navigator.share` → `navigator.clipboard.writeText` fallback, with `nativeCanShare()` for capability probing. Camera/media-pick deferred to BKL-006 (composer/media-pipeline parity) since canonical composer attachment ports are still pending and the bridge has no consumer until then. | Frontend Media Team | 2026-05-10 (share closed early) |
