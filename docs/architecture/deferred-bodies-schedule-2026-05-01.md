...
_surface adoption_ — actually rendering the BKL renderers inside the
...
-   **Quick switcher — recent-messages source: DONE.** `collectRecentMessages`
    ...
    path (`openRoomWithContext` → `roomJumpTargetEventIdAtom`). All four entry
    aspects of the exit criterion are now met.
...
-   Quick switcher returns ranked results across all four entry kinds (room/member/message/setting) with deterministic tests. _(rooms/spaces/DMs/members/pages/commands/actions/settings landed 2026-05-07; recent-messages source still pending.)_
