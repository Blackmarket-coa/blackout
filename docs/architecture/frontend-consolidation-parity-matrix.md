...
| security.auth.matrix_client_arch | apps/blackout-client | Security-core workflow control: `matrix_client_arch` | Auth/session/recovery/security flows | kept | Executable premium workflow control with release/entitlement gating for Matrix client architecture lifecycle. |
| security.auth.homeserver_discovery | apps/blackout-client | Security-core workflow control: `homeserver_discovery` | Auth/session/recovery/security flows | kept | Executable premium workflow control for homeserver discovery routing + fallback handling. |
| security.auth.e2ee_defaults | apps/blackout-client | Security-core workflow control: `e2ee_defaults` | Auth/session/recovery/security flows | kept | Executable premium workflow control for secure-by-default E2EE bootstrapping policy. |
| security.auth.oidc_delegated_auth | apps/blackout-client | Security-core workflow control: `oidc_delegated_auth` | Auth/session/recovery/security flows | kept | Executable premium workflow control for OIDC delegated auth, without baseline auth regression. |
| security.auth.matrix_bootstrap | apps/blackout-client | Security-core workflow control: `matrix_bootstrap` | Auth/session/recovery/security flows | kept | Executable premium workflow control for hardened Matrix bootstrap orchestration. |
