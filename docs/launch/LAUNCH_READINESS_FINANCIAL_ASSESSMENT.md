...
| **Can we measure the one KPI — creator-driven sales?** | **Not today.** Tips and purchases persist, but the *attribution edges* that connect a sale back to a creator/coalition/bounty/referral are ephemeral. This is the launch-critical **financial** gap. |
...
| **Bounty Board** — create, apply, complete, payout tracking | ◐ | Create/apply/complete ✅ (`routes/bounties.ts`); **reward ledger in-memory + payout deferred** (§7 fix). |
...
| **Vendor Dashboard** — products, orders, bounties, referrals, partnerships | ◐ | Products ✅; **orders/earnings not surfaced** (§7 fix). |
...
| **Referral System** — creator, vendor, coalition referrals; earnings | ◐ | Ledger exists but **in-memory**; **no `coalition` source kind**; earnings UI thin (§7 fix). |
...
**Consequence:** you can demo the loop, but you cannot produce a trustworthy month-over-month
creator-driven-sales number — which is precisely the metric you said determines whether the ecosystem is working.

**Fix (this branch):** persist the ledger → settle it on tip capture → expose
`GET /v1/growth/creator-driven-sales` + a Prometheus metric + a Creator-Hub panel (see §7).
...
## 7. The gap-close (this branch)

Four sequenced, independently-shippable changes turn "demo loop" into "measurable loop." Detail and file paths in
the implementation plan; summary:

1. **Persist the growth ledger** — move referrals/ambassadors/quests/completions/migration-credits/bounty-rewards
   from in-memory `Map`s to `db/store.ts` (mirroring `services/tips.ts`), with migrations `049–053`. No public-API
   change. *Closes the "lost on restart" defect.*
2. **Wire settlement** — settle the ledger when the reward tip captures (`tip.captured` domain event), and add the
   missing **`coalition`** referral source kind. *Closes the deferred `"PR 5b"` stub.*
3. **Creator-Driven-Sales KPI surface** — `GET /v1/growth/creator-driven-sales` + Prometheus metric + Creator-Hub
   panel + optional ops dashboard. *Makes the single KPI a first-class, queryable number.*
4. **Vendor order/earnings visibility** — read-surface of the order data Blackout already receives via the FBM
   Matrix bridge (`fbmBuyerOrderRooms` / `fbmVendorRooms`) + captured sales. *Read-only; order management stays in
   FBM.*
