...
| T5-02 | **Workstream C — Reactions / Threading parity hardening** | L (5–7 days) | **Partial progress on this branch.** Investigation found most reaction UI (`Reactions.tsx`) already ships: aggregated bar with per-reactor tooltip, single-click toggle (effectively meeting the "double-click to remove" criterion via the toggle semantics), recent-reactions in-memory, custom emoji rendering. `MessageComposer.target.mode='thread'` is wired end-to-end. `ThreadActivityPage` ships; `aggregateThreadUnread` + `applyThreadActivityUpdate` SDK helpers ship. **Landed this branch:** `recentReactionsStorage.ts` (pure module: `loadRecentReactions` / `saveRecentReactions` / `pushRecentReaction`, SSR-safe, quota-safe, MRU semantics) + wired into `Reactions.tsx`'s `useState` lazy init + `sendReaction` persistence. 19 unit tests covering load/save round-trip, malformed JSON tolerance, dedup, clamping, blank handling. **Still open:** in-room thread panel rendering root + replies + composer-in-thread-mode (right-panel "threads" slot currently shows a flat event list); thread unread badge in the left-panel; reaction picker keyboard navigation; integration-level tests for reaction add/remove/aggregate + thread reply post + unread badge update. Depends on T4-01 (closed). Can run alongside Workstream B once foundation primitives ship. |
