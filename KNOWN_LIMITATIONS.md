...
| **D** | Discord parity P2: GIF picker (Giphy), voice/video polish, screen-sharing polish, media player controls. | **Largely closed (2026-07-12).** GIF picker shipped in full (Giphy provider + Tenor fallback + per-device recents). Voice messages now round-trip: the composer sends `m.audio` + MSC3245 with real waveform/duration metadata (previously fell through as `m.file`), and the timeline player renders transmitted peaks. Media players gained playback-speed (audio+video) and audio volume controls; screen sharing gained a local preview tile. Remaining: the real MatrixRTC/LiveKit runtime binding (infra-coupled — see `features/call/README.md`) and multi-party call UX polish. |
