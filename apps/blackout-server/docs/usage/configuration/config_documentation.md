...
_The "untracked" option was added in Synapse 1.96.0._
...
- `ssh_priv_key_path` and `ssh_pub_key_path`: The private and public SSH key pair used to encrypt the manhole traffic.
  Upstream Synapse falls back to a keypair hardcoded in its source when these
  are unset — published, therefore not secret, and identical on every install.
  **This build has removed that key** and generates a fresh one per process
  instead, logging a warning when it does. Leaving these unset is no longer a
  confidentiality problem, but the host key then changes on every restart and
  your ssh client will say so. Set them to pin a stable, genuinely private key.
  See [the manhole documentation](../../manhole.md) for the reasoning.
...
- `require_transport_security`: Set to true to require TLS transport security for SMTP.
  ...
  TLS via STARTTLS _if the SMTP server supports it_. If this option is set,
...
- `sync_response_cache_duration`: Controls how long the results of a /sync request are
  ...
  _Changed in Synapse 1.62.0_: The default was changed from 0 to 2m.
...
- `allow_unsafe_locale` is an option specific to Postgres. Under the default behavior, Synapse will refuse to
  start if the postgres db is set to a non-C locale. You can override this behavior (which is _not_ recommended)
...
The `rc_invites.per_user` limit applies to the _receiver_ of the invite, rather than the
sender, meaning that a `rc_invite.per_user.burst_count` of 5 mandates that a single user
cannot _receive_ more than a burst of 5 invites at a time.

In contrast, the `rc_invites.per_issuer` limit applies to the _issuer_ of the invite, meaning that a `rc_invite.per_issuer.burst_count` of 5 mandates that single user cannot _send_ more than a burst of 5 invites at a time.
...
How many _pending media uploads_ can a given user have? A pending media upload
...
Enable registration without email or captcha verification. Note: this option is _not_ recommended,
...
_Deprecated in Synapse 1.64.0_: The `email` option is deprecated.

_Removed in Synapse 1.66.0_: The `email` option has been removed.
...
It _must_ be configured if `autocreate_auto_join_room_preset` is set to
...
_Changed in Synapse 1.74:_ admins can filter the events in prejoin state based
on their state key.
...
admin user), and _not_ the puppeted user.
...
_New in Synapse 1.67_: If this file does not exist, Synapse will create a new signing
...
- `user_mapping_provider`: Configuration for how attributes returned from a OIDC
  ...
  - `config`: Configuration for the mapping provider module. This section will
    ...
    - `subject_claim`: name of the claim containing a unique identifier
      ...
      _Deprecated in Synapse v1.75.0._
...
- `group_unread_count_by_room: false`: When a push notification is received, an unread count is also sent.
  This number can either be calculated as the number of unread messages for the user, or the number of _rooms_ the
...
- `server_notice_content`: if enabled, will send a user a "Server Notice"
  ...
  must also be configured for this to work. Notices will _not_ be sent to
...
If you use this setting, you _must_ define the `system_mxid_localpart`
...
Configuration for Redis when using workers. This _must_ be enabled when using workers.
This setting has the following sub-options:
...
- `ca_path`: Optional path to the folder containing the CA certificate file
  ...
  _Changed in Synapse 1.84.0: Added use_tls, certificate_file, private_key_file, ca_file and ca_path attributes_
