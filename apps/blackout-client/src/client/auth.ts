...
const normalizeAuthError = (error: unknown): MatrixInitError => {
  ...
  switch (matrixError?.errcode) {
    ...
    default:
      ...
      return new MatrixInitError('network_failure', matrixError?.message ?? 'Authentication failed.');
  }
};
...
const saveFromLoginResponse = (
  ...
  if (!response.access_token || !response.user_id || !response.device_id) {
    throw new MatrixInitError('invalid_credentials', 'Login response did not include required session fields.');
  }
...
export const loginWithPassword = async (store: AtomStore, input: PasswordLoginInput): Promise<MatrixClient> => {
  ...
  try {
    ...
    if (!initializedClient) {
      throw new MatrixInitError('invalid_credentials', 'Unable to restore session after login.');
    }
...
export const registerUser = async (store: AtomStore, input: RegistrationInput): Promise<MatrixClient> => {
  store.set(authStateAtom, 'loading');
  ...
  try {
    ...
    const initializedClient = await initMatrixFromStoredSession(store);
    if (!initializedClient) {
      throw new MatrixInitError('invalid_credentials', 'Unable to restore session after registration.');
    }
...
export const refreshSessionToken = async (store: AtomStore): Promise<StoredSession> => {
  ...
  if (!session?.refreshToken) {
    throw new MatrixInitError('invalid_credentials', 'No refresh token available for active session.');
  }
  ...
  if (!response.ok) {
    if (response.status === 429) throw new MatrixInitError('rate_limited', 'Token refresh is rate-limited.');
