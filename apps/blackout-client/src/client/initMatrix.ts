...
const initClientForSession = async (session: StoredSession): Promise<MatrixClient> => {
  ...
  const cryptoStore = new IndexedDBCryptoStore(window.indexedDB, `blackout-crypto-store:${session.userId}`);
...
export const initMatrixFromStoredSession = async (store: AtomStore): Promise<MatrixClient | null> => {
