...
/**
 * A Matrix id reduced to something readable: the localpart, or the raw id when
 * it does not parse. The fallback every surface that renders a bare user id
 * wants — several had each rolled their own regex for it.
 */
export const mxIdToDisplayName = (userId: string): string => getMxIdLocalPart(userId) ?? userId;
