...
export function PasswordRegisterForm({
  ...
  useRegisterComplete(customRegisterResp, (path) => navigate(path, { replace: true }));
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (evt) => {
    ...
    const pickedStages = pickStages(uiaFlows, nextFormData);
    ...
    <>
      <Box as="form" onSubmit={handleSubmit} direction="Inherit" gap="400">
              ...
              <Box direction="Column" gap="100">
                ...
                {registerError?.errcode === RegisterError.PasswordWeak && (
                  <FieldError
                    message={
                      ...
                      'Weak password. Password rejected by server, please choose a stronger password.'
                    }
                  />
                )}
                {registerError?.errcode === RegisterError.PasswordShort && (
                  <FieldError
                    message={
                      registerError.data.error ??
                      'Short password. Password rejected by server, please choose a longer password.'
                    }
                  />
                )}
              </Box>
        ...
        {registerError?.errcode === RegisterError.RateLimited && (
          <FieldError message="Failed to register. Your request has been rate-limited by the server." />
        )}
