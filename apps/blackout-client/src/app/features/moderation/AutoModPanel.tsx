...
const parseKeywordFilters = (value: unknown): KeywordFilterRule[] => {
    ...
    return value
        .map((item) => {
            ...
            if (record.type !== 'exact' && record.type !== 'wildcard' && record.type !== 'regex')
                return null;
            if (
                record.action !== 'warn' &&
...
const parseConfig = (content: Record<string, unknown> | undefined): AutoModConfig => {
    ...
    const spamRaw = (
        content.spamProtection && typeof content.spamProtection === 'object'
            ? content.spamProtection
            : {}
    ) as Record<string, unknown>;

    const raidRaw = (
        content.raidProtection && typeof content.raidProtection === 'object'
            ? content.raidProtection
            : {}
    ) as Record<string, unknown>;

    const newAccountsRaw = (
        content.newAccountRestrictions && typeof content.newAccountRestrictions === 'object'
    ...
    return {
        ...
        spamProtection: {
            enabled:
                spamRaw.enabled === undefined
                    ? defaultConfig.spamProtection.enabled
                    : spamRaw.enabled === true,
            maxMessagesPerMinute:
                typeof spamRaw.maxMessagesPerMinute === 'number'
                    ? Math.max(1, Math.round(spamRaw.maxMessagesPerMinute))
                    : defaultConfig.spamProtection.maxMessagesPerMinute,
            duplicateThreshold:
                typeof spamRaw.duplicateThreshold === 'number'
                    ? Math.max(1, Math.round(spamRaw.duplicateThreshold))
                    : defaultConfig.spamProtection.duplicateThreshold,
            linkWhitelist: Array.isArray(spamRaw.linkWhitelist)
                ? spamRaw.linkWhitelist.filter(
                      (entry): entry is string => typeof entry === 'string',
                  )
                : defaultConfig.spamProtection.linkWhitelist,
        },
        raidProtection: {
            enabled:
                raidRaw.enabled === undefined
                    ? defaultConfig.raidProtection.enabled
                    : raidRaw.enabled === true,
            joinRateThreshold:
                typeof raidRaw.joinRateThreshold === 'number'
                    ? Math.max(1, Math.round(raidRaw.joinRateThreshold))
                    : defaultConfig.raidProtection.joinRateThreshold,
            autoLockdown:
                raidRaw.autoLockdown === undefined
                    ? defaultConfig.raidProtection.autoLockdown
                    : raidRaw.autoLockdown === true,
        },
        newAccountRestrictions: {
            enabled:
                newAccountsRaw.enabled === undefined
                    ? defaultConfig.newAccountRestrictions.enabled
                    : newAccountsRaw.enabled === true,
            minAgeHours:
                typeof newAccountsRaw.minAgeHours === 'number'
                    ? Math.max(0, Math.round(newAccountsRaw.minAgeHours))
                    : defaultConfig.newAccountRestrictions.minAgeHours,
            restrictedActions: Array.isArray(newAccountsRaw.restrictedActions)
                ? newAccountsRaw.restrictedActions.filter(
                      (entry): entry is string => typeof entry === 'string',
                  )
            ...
            >
                {activeTab === 'keywords' ? (
                    <KeywordFilterEditor
                        ...
                        onChange={(rules) =>
                            setConfig((prev) => ({ ...prev, keywordFilters: rules }))
                        }
                    />
                ) : null}
                            ...
                            <input
                                ...
                                onChange={(event) =>
                                    setConfig((prev) => ({
                                        ...
                                        raidProtection: {
                                            ...prev.raidProtection,
                                            autoLockdown: event.target.checked,
                                        },
                                    }))
                                }
                            />{' '}
