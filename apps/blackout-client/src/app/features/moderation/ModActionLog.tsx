...
export const ModActionLog = ({ managementRoomId }: { managementRoomId: string }) => {
  ...
  const actionOptions = useMemo(() => ['all', ...new Set(entries.map((entry) => entry.action))], [entries]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return entries.filter((entry) => {
      if (actionFilter !== 'all' && entry.action !== actionFilter) return false;
      if (moderatorFilter && !entry.moderator.toLowerCase().includes(moderatorFilter.toLowerCase())) return false;
      if (targetFilter && !entry.target.toLowerCase().includes(targetFilter.toLowerCase())) return false;
      if (!q) return true;
      return `${entry.action} ${entry.moderator} ${entry.target} ${entry.reason}`.toLowerCase().includes(q);
    });
  ...
  return (
    <section style={{ display: 'grid', gap: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8 }}>
        ...
        <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
          Moderator
          <input value={moderatorFilter} onChange={(event) => setModeratorFilter(event.target.value)} placeholder="@mod:server" />
        </label>
        <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
          Target user
          <input value={targetFilter} onChange={(event) => setTargetFilter(event.target.value)} placeholder="@target:server" />
        </label>
        <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
          Search
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="reason or action" />
        </label>
      </div>

      <div style={{ border: '1px solid var(--border-default)', borderRadius: 10, overflow: 'hidden' }}>
        {filtered.length === 0 ? (
          <div style={{ padding: 10, fontSize: 12, color: 'var(--text-secondary)' }}>No moderation log entries found.</div>
        ) : (
          filtered.map((entry, index) => (
            <article key={entry.eventId} style={{ padding: 10, borderTop: index === 0 ? 'none' : '1px solid var(--border-default)', display: 'grid', gap: 4, background: 'var(--bg-input)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                <strong>{entry.action}</strong>
                <time style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{new Date(entry.timestamp).toLocaleString()}</time>
              </div>
