...
export const TimeoutBadge = ({ userId, roomId }: { userId: string; roomId: string }) => {
  ...
  return (
    <span title={`Timed out until ${new Date(activeTimeout.expiresAt).toLocaleString()}`} style={{ fontSize: 10, border: '1px solid var(--danger)', color: 'var(--danger)', borderRadius: 999, padding: '1px 6px' }}>
      Timed out
    </span>
  );
};
...
export const TimeoutDialog = ({
  ...
  const durationMs = preset === 'custom' ? Math.max(1, customMinutes) * 60 * 1000 : PRESET_TO_MS[preset];
  ...
  const applyTimeout = async () => {
    ...
    try {
      ...
      const currentUsers = (content.users && typeof content.users === 'object' ? content.users : {}) as Record<string, number>;
      ...
      const existing = parseTimeouts(timeoutEvent?.getContent<Record<string, unknown>>()).filter(
        (entry) => !(entry.userId === targetUserId && entry.expiresAt > Date.now()),
      );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', zIndex: 40 }} onClick={onClose}>
      <div
        style={{ width: 420, maxWidth: '95vw', margin: '12vh auto', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 12, padding: 12, display: 'grid', gap: 10 }}
        onClick={(event) => event.stopPropagation()}
      >
        <h3 style={{ margin: 0 }}>Timeout user</h3>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--text-secondary)' }}>{targetUserId}</p>
        ...
        {preset === 'custom' ? (
          <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
            ...
            <input type="number" min={1} value={customMinutes} onChange={(event) => setCustomMinutes(Math.max(1, Number(event.target.value) || 1))} />
          </label>
        ) : null}

        <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
          Reason
          <textarea rows={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Optional reason" />
        </label>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button type="button" onClick={onClose} style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', padding: '6px 10px' }}>
            Cancel
          </button>
          <button
            type="button"
            ...
            disabled={saving}
            style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--danger)', color: '#fff', padding: '6px 10px' }}
          >
...
const restoreTimeout = async (roomId: string, userId: string, previousPower: number, client: MatrixClient) => {
