  ...
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button type="button" onClick={() => onChange([...rules, blankRule()])} style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', padding: '4px 8px' }}>
          Add rule
        </button>
      </div>

      {rules.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>No rules yet. Add your first filter rule.</div> : null}

      {rules.map((rule, index) => (
        <article key={`${index}-${rule.pattern}`} style={{ border: '1px solid var(--border-default)', borderRadius: 10, padding: 10, display: 'grid', gap: 6 }}>
          <div style={{ display: 'grid', gap: 4 }}>
            <label style={{ fontSize: 12 }}>Pattern</label>
            <input
              value={rule.pattern}
              onChange={(event) =>
                onChange(rules.map((candidate, candidateIndex) => (candidateIndex === index ? { ...candidate, pattern: event.target.value } : candidate)))
              }
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                onChange={(event) =>
                  onChange(
                    rules.map((candidate, candidateIndex) =>
                      candidateIndex === index ? { ...candidate, type: event.target.value as KeywordFilterType } : candidate,
                    ),
                  )
                }
              >
            ...
            <label style={{ display: 'grid', gap: 2, fontSize: 12 }}>
              ...
              <select
                ...
                onChange={(event) =>
                  onChange(
                    rules.map((candidate, candidateIndex) =>
                      candidateIndex === index ? { ...candidate, action: event.target.value as KeywordFilterAction } : candidate,
                    ),
                  )
                }
              >
            ...
            <button
              type="button"
              onClick={() => onChange(rules.filter((_, candidateIndex) => candidateIndex !== index))}
              style={{ marginTop: 18, border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--danger)', color: '#fff', padding: '4px 8px' }}
            >
              Remove
            </button>
          </div>
      ...
      <fieldset style={{ border: '1px solid var(--border-default)', borderRadius: 10, padding: 10 }}>
        <legend style={{ fontSize: 12 }}>Test patterns</legend>
        <textarea rows={3} value={sampleText} onChange={(event) => setSampleText(event.target.value)} />
        <div style={{ marginTop: 8, display: 'grid', gap: 4 }}>
          {testResults.map(({ rule, matched }, index) => (
            <div key={`${index}-${rule.pattern}`} style={{ fontSize: 12, color: matched ? 'var(--danger)' : 'var(--text-secondary)' }}>
              {matched ? 'Matched' : 'No match'} · <code>{rule.type}</code> · <code>{rule.action}</code> · {rule.pattern || '(empty)'}
            </div>
          ))}
        </div>
      </fieldset>
      ...
      <fieldset style={{ border: '1px solid var(--border-default)', borderRadius: 10, padding: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button
            type="button"
            onClick={() => triggerDownload('automod-keyword-filters.json', JSON.stringify(rules, null, 2))}
          ...
          <button
            ...
            onClick={() => {
              try {
                ...
                if (!Array.isArray(parsed)) throw new Error('Import payload must be an array of keyword rules.');
            ...
            style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--accent-primary)', color: 'var(--bg-surface)', padding: '4px 8px' }}
          >
            Import JSON
          </button>
          {importError ? <span style={{ color: 'var(--danger)', fontSize: 12 }}>{importError}</span> : null}
        </div>
      </fieldset>
    </div>
  );
