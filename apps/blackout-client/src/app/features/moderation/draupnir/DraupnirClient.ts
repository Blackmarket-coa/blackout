...
const fromTimelineEvent = (event: MatrixEvent): { content: Record<string, unknown>; timestamp: number } => {
...
const parseAction = (event: MatrixEvent): DraupnirModAction | null => {
  ...
  const action = toString(content.action) ?? toString(content.outcome) ?? toString(content.recommendation);
  if (!action) return null;

  const moderator = event.getSender() ?? toString(content.moderator) ?? 'unknown';
  const target = toString(content.target) ?? toString(content.user_id) ?? toString(content.entity);
...
const parseBanEntry = (event: MatrixEvent): DraupnirBanEntry | null => {
  const { content, timestamp } = fromTimelineEvent(event);
  const candidate =
    toString(content.entity) ?? toString(content.target) ?? toString(content.user_id) ?? toString(content.server) ?? toString(content.pattern);
  ...
  return {
    entityType: toString(content.entity_type) as DraupnirBanEntity | undefined ?? inferEntityType(candidate),
    value: candidate,
...
const parseProtection = (event: MatrixEvent): DraupnirProtectionState | null => {
  ...
  const enabled = toBoolean(content.enabled) ?? (/enabled|active/i.test(toString(content.status) ?? ''));
...
export class DraupnirClient {
  ...
  public async findManagementRoom(): Promise<Room | null> {
    ...
    if (this.config?.managementRoomAlias) {
      const byAlias = this.matrixClient.getRooms().find((room) => room.getCanonicalAlias() === this.config?.managementRoomAlias);
    ...
    return (
      this.matrixClient
        ...
        .find((room) => {
          ...
          return alias.includes('draupnir') || name.includes('draupnir') || name.includes('moderation management');
        }) ?? null
    );
  }
