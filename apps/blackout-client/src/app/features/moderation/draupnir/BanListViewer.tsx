...
export const BanListViewer = ({ config }: { config?: DraupnirClientConfig }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return snapshot.banEntries.filter((entry) => {
      return `${entry.value} ${entry.reason ?? ''} ${entry.sourceRoomId ?? ''}`.toLowerCase().includes(q);
    });
  }, [filter, snapshot]);
  ...
  if (!snapshot) {
    return <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Draupnir management room not found.</div>;
  }
    ...
    <section style={{ display: 'grid', gap: 10 }}>
      <header style={{ display: 'grid', gap: 4 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
          Policy list rooms watched: {snapshot.policyListRooms.length > 0 ? snapshot.policyListRooms.join(', ') : 'none discovered'}
        </div>
      </header>

      <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
        Search bans
        <input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="user/server/pattern/reason" />
      </label>

      <div style={{ border: '1px solid var(--border-default)', borderRadius: 10, overflow: 'hidden' }}>
        {filtered.length === 0 ? (
          <div style={{ padding: 10, fontSize: 12, color: 'var(--text-secondary)' }}>No ban entries detected from Draupnir output.</div>
        ) : (
          filtered.map((entry, index) => (
            <article key={entry.eventId} style={{ padding: 10, borderTop: index === 0 ? 'none' : '1px solid var(--border-default)', display: 'grid', gap: 4 }}>
              ...
              {entry.reason ? <div style={{ fontSize: 12 }}><strong>Reason:</strong> {entry.reason}</div> : null}
              <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
