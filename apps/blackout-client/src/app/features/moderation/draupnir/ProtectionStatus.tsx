...
export const ProtectionStatus = ({ config }: { config?: DraupnirClientConfig }) => {
  ...
  const toggle = async (key: string, nextEnabled: boolean) => {
    ...
    setPendingKey(key);
    try {
      await draupnir.sendCommand(snapshot.roomId, 'protection', [nextEnabled ? 'enable' : 'disable', key]);
    } finally {
      setPendingKey(null);
    }
  };

  if (!snapshot) return <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>No Draupnir room found.</div>;
    ...
    <section style={{ display: 'grid', gap: 10 }}>
      ...
      <div style={{ border: '1px solid var(--border-default)', borderRadius: 10, overflow: 'hidden' }}>
        {snapshot.protections.length === 0 ? (
          <div style={{ padding: 10, fontSize: 12, color: 'var(--text-secondary)' }}>No protection status discovered yet.</div>
        ) : (
          snapshot.protections.map((protection, index) => (
            <article key={protection.sourceEventId} style={{ padding: 10, borderTop: index === 0 ? 'none' : '1px solid var(--border-default)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={indicatorStyle(protection.enabled)} />
                <strong>{protection.key}</strong>
                <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{protection.enabled ? 'active' : 'disabled'}</span>
              </div>
              <button
                type="button"
              ...
              >
                {pendingKey === protection.key ? 'Sending…' : protection.enabled ? 'Disable' : 'Enable'}
              </button>
            </article>
          ))
        )}
