...
export const ProposalCard = ({
  ...
  return (
    <article style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}>
        <div>
          <strong>{proposal.title}</strong>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>by {proposal.authorId}</div>
        </div>
        <span style={{ background: statusColor[proposal.status], color: '#fff', borderRadius: 999, padding: '2px 8px', fontSize: 12 }}>
          {proposal.status}
        </span>
      </div>
      ...
      <div style={{ display: 'grid', gap: 4 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{voteCount}/{proposal.quorum} votes</div>
      </div>
