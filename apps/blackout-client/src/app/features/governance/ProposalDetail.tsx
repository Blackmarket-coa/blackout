...
export const ProposalDetail = ({
  ...
  const myVote = useMemo(() => votes.data.find((vote) => vote.voterId === currentUserId) ?? null, [currentUserId, votes.data]);

  if (!proposal) {
    return <section style={{ padding: 12, color: 'var(--text-secondary)' }}>Proposal not found.</section>;
  }
    ...
    <section style={{ display: 'grid', gap: 12 }}>
      <header style={{ display: 'grid', gap: 4 }}>
        ...
        <div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>
          Status: {result.data?.computedStatus} • Votes: {result.data?.voteCount ?? 0}/{proposal.quorum}
        </div>
      </header>

      <article style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12 }}>
        <pre style={{ margin: 0, whiteSpace: 'pre-wrap', font: 'inherit' }}>{proposal.description}</pre>
      </article>
      ...
      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 8 }}>
          ...
          {proposal.options.map((option) => {
            ...
            return (
              <button
                ...
                style={{ border: `1px solid ${selected ? 'var(--accent-primary)' : 'var(--border-default)'}`, borderRadius: 10, background: selected ? 'var(--accent-muted)' : 'var(--bg-input)', color: 'var(--text-primary)', textAlign: 'left', padding: '8px 10px' }}
              >
                {option.label}
              </button>
            );
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
            {myVote ? `Current vote: ${typeof myVote.choice === 'string' ? myVote.choice : myVote.choice.join(', ')}` : 'No vote cast yet'}
          </span>
          <button
            type="button"
            onClick={() => void submitVote()}
            disabled={selectedChoices.length === 0 || submitting || proposal.status !== 'active'}
      ...
      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {proposal.options.map((option) => {
            const resultItem = optionResults.find((item) => item.optionId === option.id);
      ...
      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 8 }}>
        ...
        {anonymousMode ? (
          <span style={{ color: 'var(--text-secondary)', fontSize: 12 }}>Anonymous mode enabled.</span>
        ) : (
          <ul style={{ margin: 0, paddingLeft: 18, color: 'var(--text-secondary)' }}>
            {votes.data.map((vote) => (
              <li key={vote.eventId}>
                {vote.voterId} → {typeof vote.choice === 'string' ? vote.choice : vote.choice.join(', ')}
              </li>
            ))}
          </ul>
        )}
