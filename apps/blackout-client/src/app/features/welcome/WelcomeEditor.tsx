...
export const WelcomeEditor = ({ spaceId }: { spaceId: string }) => {
  ...
  const uploadBanner = async (file: File): Promise<string | undefined> => {
    const uploadResult = (await client.uploadContent(file, { includeFilename: true, type: file.type })) as string | { content_uri?: string };
    if (typeof uploadResult === 'string') return uploadResult;
    return uploadResult.content_uri;
  };

  const addFeaturedChannel = () => setDraft((prev) => ({ ...prev, featuredChannels: [...prev.featuredChannels, { ...emptyChannel }] }));
  ...
  if (preview) {
    return (
      <section style={{ display: 'grid', gap: 12 }}>
        ...
        <OnboardingWizard spaceId={spaceId} open={onboardingDraft.enabled} onClose={() => undefined} />
      </section>
    );
  }
      ...
      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 10 }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          {draft.featuredChannels.map((channel, index) => (
            <article key={`${channel.roomId}-${index}`} style={{ border: '1px solid var(--border-default)', borderRadius: 10, background: 'var(--bg-input)', padding: 8, display: 'grid', gap: 6 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 8 }}>
                <input
                  ...
                  placeholder="Emoji"
                  style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-surface)', color: 'var(--text-primary)', padding: '6px 8px' }}
                />
                <input
              ...
              >
                ...
                {memberRoomOptions.map((option) => (
                  <option key={option.roomId} value={option.roomId}>{option.label}</option>
                ))}
              </select>
            </article>
          ))}
      ...
      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12, display: 'grid', gap: 10 }}>
        ...
        <label style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
          <input
            ...
            onChange={(event) => setOnboardingDraft((prev) => ({ ...prev, enabled: event.target.checked }))}
          />
          Enable onboarding wizard
        </label>

        <div style={{ display: 'inline-flex', gap: 8 }}>
          <button type="button" onClick={() => addStep('rules')} style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--text-primary)', padding: '4px 8px' }}>Add Rules</button>
          <button type="button" onClick={() => addStep('roles')} style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--text-primary)', padding: '4px 8px' }}>Add Roles</button>
          <button type="button" onClick={() => addStep('channels')} style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--text-primary)', padding: '4px 8px' }}>Add Channels</button>
        </div>

        {onboardingDraft.steps.map((step, index) => (
          <article key={`${step.type}-${index}`} style={{ border: '1px solid var(--border-default)', borderRadius: 10, background: 'var(--bg-input)', padding: 8, display: 'grid', gap: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--danger)', color: '#fff', padding: '3px 8px' }}
              >
                Delete
              </button>
            </div>
            ...
            {step.type === 'rules' ? (
              <>
                <textarea
                  ...
                  style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-surface)', color: 'var(--text-primary)', padding: 8 }}
                />
                <label style={{ display: 'inline-flex', gap: 6 }}>
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      setOnboardingDraft((prev) => {
                        const steps = [...prev.steps];
                        steps[index] = { ...steps[index], requireAccept: checked };
                        return { ...prev, steps };
                      });
                    }}
                  />
            ...
            {step.type === 'roles' ? (
              <textarea
                ...
                onChange={(event) => {
                  const roles = event.target.value.split(',').map((item) => item.trim()).filter(Boolean);
                  setOnboardingDraft((prev) => {
                    const steps = [...prev.steps];
            ...
            {step.type === 'channels' ? (
              <textarea
                ...
                onChange={(event) => {
                  const channels = event.target.value.split(',').map((item) => item.trim()).filter(Boolean);
                  setOnboardingDraft((prev) => {
                    const steps = [...prev.steps];
