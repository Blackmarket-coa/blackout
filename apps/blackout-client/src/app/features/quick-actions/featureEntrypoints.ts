...
export function buildFeatureEntrypointRegistry(options: BuildRegistryOptions = {}): FeatureEntrypointRegistry {
...
export function getQuickActionEntriesForSurface(registry: FeatureEntrypointRegistry, surface: QuickActionSurface): FeatureEntry[] {
...
export function getUnseenQuickActionIds(visibleEntries: FeatureEntry[]): QuickActionId[] {
  ...
  const seen = new Set<QuickActionId>((raw ? (JSON.parse(raw) as QuickActionId[]) : []).filter(Boolean));
...
export function invokeQuickAction(actionId: QuickActionId, context: QuickActionInvocationContext): void {
  switch (actionId) {
    case 'open-settings':
