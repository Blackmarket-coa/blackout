...
describe('feature entrypoint registry adapter', () => {
  ...
  it('respects preset + flag visibility', () => {
    ...
    const governanceWithSearch = buildFeatureEntrypointRegistry({ preset: 'governance', flags: { 'features.nav.search': true } });
    expect(governanceWithSearch.entries.map((entry) => entry.id)).toEqual(['open-settings', 'open-devices', 'open-inbox', 'open-search']);
  });
  ...
  it('maintains mobile/desktop render parity and first-run guidance behavior', () => {
    ...
    const desktopIds = getQuickActionEntriesForSurface(registry, 'desktop').map((entry) => entry.id);
    const mobileIds = getQuickActionEntriesForSurface(registry, 'mobile').map((entry) => entry.id);
