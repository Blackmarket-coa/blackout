...
import { fabStackBottom, usePageFabStackOffset } from '../../hooks/useFabStack';
...
const buttonStyle: CSSProperties = {
    ...
    // A page-level compose FAB (Coliseum etc.) owns that base slot when present,
    // so `bottom` is overridden per-render to stack above it.
...
export const BugReportFab = () => {
    ...
    const stackOffset = usePageFabStackOffset();
    ...
    return (
        <>
            {!open && !keyboardOpen && (
                <button
                    type="button"
                    style={{ ...buttonStyle, bottom: fabStackBottom(stackOffset) }}
