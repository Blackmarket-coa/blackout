...
export function ProfilePublicSettings() {
    ...
    // Opt-OUT, surfaced as an opt-IN switch: listed unless the member says no.
    const [listedOnRosters, setListedOnRosters] = useState<boolean>(true);
    ...
    useEffect(() => {
        ...
        fetchProfile(userId)
            .then((member) => {
                ...
                setListedOnRosters(p.hideFromPublicRosters !== true);
    ...
    const addSponsor = useCallback(() => {
        ...
        if (!candidate) return;
        setSponsors((prev) =>
            prev.includes(candidate) ? prev : [...prev, candidate].slice(0, 12)
        );
        setNewSponsor('');
    }, [newSponsor]);
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            const merged: BmcProfileEvent = {
                ...current,
                public: isPublic,
                hideFromPublicRosters: listedOnRosters ? undefined : true,
                connections,
    ...
    }, [userId, isPublic, listedOnRosters, fbmHandle, sponsors]);
    ...
    return (
        <Box direction="Column" gap="100">
            ...
            >
                ...
                <SettingTile
                    title="List me on coalition rosters"
                    description="Coalitions are public, so people can find and join them. Turn this off to stay out of public rosters, member counts and supporter lists. People already in the coalition still see you."
                    after={
                        <Switch
                            variant="Primary"
                            value={listedOnRosters}
                            onChange={setListedOnRosters}
                        />
                    }
                />
                ...
                <Box gap="200" alignItems="Center">
                    ...
                    {saveState === 'saved' && (
                        <Text
                            size="T200"
                            style={{ color: color.Success.Main, padding: `0 ${config.space.S200}` }}
                        >
                            Saved.
                        </Text>
                    )}
