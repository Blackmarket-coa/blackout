      ...
      <section>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          {settings.blockedUsers.map((user) => (
            <div key={user.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid var(--border-default)', borderRadius: 8, padding: 8 }}>
              <div>
              ...
              <button
                ...
                onClick={() =>
                  setSettings((prev) => ({
                    ...prev,
                    blockedUsers: prev.blockedUsers.filter((entry) => entry.id !== user.id),
                  }))
                }
              >
                Unblock
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {dmOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => setSettings((prev) => ({ ...prev, dmPermissions: option.value }))}
              style={{
                border: settings.dmPermissions === option.value ? '1px solid var(--accent-primary)' : '1px solid var(--border-default)',
                borderRadius: 8,
                padding: '6px 10px',
                background: settings.dmPermissions === option.value ? 'var(--bg-surface-hover)' : 'var(--bg-input)',
