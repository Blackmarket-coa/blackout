...
export function DiscoverySurface() {
  ...
  const selectedItem = discoveryItems.find((item) => item.roomId === selectedRoomId) ?? discoveryItems[0];

  const onItemKeyDown = (index: number): KeyboardEventHandler<HTMLButtonElement> => (evt) => {
              ...
              <Box gap="100" wrap="Wrap">
                ...
                {(['all', 'joined', 'joinable', 'invite'] as const).map((value) => (
                  <Chip
                    ...
                    onClick={() => setFilters((prev) => ({ ...prev, access: value }))}
                  >
                ...
                {(['all', 'active', 'quiet'] as const).map((value) => (
                  <Chip
                    ...
                    onClick={() => setFilters((prev) => ({ ...prev, activity: value }))}
                  >
              ...
              <Box direction="Column" gap="100" style={{ flex: '2 1 0', minWidth: 0 }}>
                <Scroll hideTrack visibility="Hover" size="0">
                  <Box direction="Column" gap="100" role="listbox" aria-label="Discovery results">
                    ...
                    {!isLoading && discoveryItems.length === 0 && <Text>No discovery results found.</Text>}
                    {discoveryItems.map((item, index) => (
                      ...
                      >
                        <Box as="span" alignItems="Center" gap="200" grow="Yes">
                          <Avatar size="200">
                            <Icon src={item.roomType === RoomType.Space ? Icons.Space : Icons.Hash} />
                          </Avatar>
                          <Box as="span" direction="Column" alignItems="Start" grow="Yes">
                            <Text size="L400" truncate>{item.name}</Text>
                            <Text size="T200" truncate>{item.memberCount} members</Text>
                          </Box>
                          {item.inHierarchy && <Chip variant="Success"><Text size="T200">In hierarchy</Text></Chip>}
                        </Box>
                      </Button>
                    ))}
                  </Box>
              ...
              <Box direction="Column" gap="200" style={{ flex: '1 1 0', minWidth: 0 }}>
                ...
                {selectedItem ? (
                  <Box direction="Column" gap="100">
                    ...
                    <Text size="T200">Type: {selectedItem.roomType === MatrixRoomType.Space ? 'Space' : 'Room'}</Text>
                    <Text size="T200">Parent spaces: {selectedItem.parentSpaceIds.length}</Text>
                    <Button variant="Primary" onClick={() => openOrJoin(selectedItem)}>
                      <Text size="B300">{selectedItem.joined ? 'Open' : 'Join & Open'}</Text>
                    </Button>
                  </Box>
                ) : (
