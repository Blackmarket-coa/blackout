...
} from '../../components/bmc/auth/homeserver';
import { isLikelySandbox } from './model';
...
const fetchPublicRooms = async (): Promise<LoadState> => {
    ...
    try {
        ...
        return {
            kind: 'error',
            message: 'Could not load public rooms. Check your connection and try again.',
        };
    }
        ...
        >
            <section style={cardStyle}>
                ...
                <p style={{ margin: 0, opacity: 0.85, fontSize: 14 }}>
                    Explore the community before you join. Sign in or create an account to
                    participate.
                </p>
                ...
                {state.kind === 'loading' ? (
                    <p style={{ margin: 0 }}>Loading public rooms…</p>
                ) : null}
                ...
                {state.kind === 'error' ? (
                    <p style={{ margin: 0, color: 'var(--text-critical, #f87171)' }}>
                        {state.message}
                    </p>
                ) : null}
                        ...
                        {visibleRooms.map((room) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {/* This list stays unfiltered (no toggle pre-login), so label
                        likely dev/sandbox rooms instead of hiding them. */}
                                        {isLikelySandbox({ memberCount: room.members })
                                            ? 'Sandbox · '
                                            : null}
                                ...
                                {room.alias ? (
                                    <div style={{ fontSize: 12, opacity: 0.65 }}>{room.alias}</div>
                                ) : null}
                                {room.topic ? (
