...
export const QuickSwitcher = ({ open, onClose, onCommandPicked }: QuickSwitcherProps) => {
  ...
  const flattened = useMemo(
    () => (Object.keys(grouped) as Array<keyof typeof grouped>).flatMap((category) => grouped[category]),
    [grouped],
  );
          ...
          {(Object.keys(grouped) as Array<keyof typeof grouped>).map((category) => {
            ...
            return (
              <div key={category} style={{ marginBottom: 12 }}>
                <div style={{ color: 'var(--text-muted)', fontSize: 11, textTransform: 'uppercase', padding: '4px 8px' }}>{category}</div>
                {items.map((item) => {
                  const absoluteIndex = flattened.findIndex((entry) => entry.id === item.id && entry.category === item.category);
                  ...
                  return (
                    <button
                      ...
                      style={{ width: '100%', textAlign: 'left', border: 'none', borderRadius: 8, padding: '8px 10px', display: 'grid', gridTemplateColumns: '28px 1fr auto', gap: 10, alignItems: 'center', background: active ? 'var(--accent-muted)' : 'transparent', color: 'var(--text-primary)' }}
                    >
                      <span style={{ width: 24, height: 24, borderRadius: 999, background: 'var(--bg-input)', display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
                        {item.avatarUrl ? <img src={item.avatarUrl} alt={item.title} style={{ width: '100%', height: '100%' }} /> : item.category === 'Commands' ? '⌘' : item.category === 'Users' ? '👤' : item.category === 'Spaces' ? '🗂️' : '💬'}
                      </span>
                      <span style={{ minWidth: 0 }}>
                        <span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</span>
                        {item.subtitle ? <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>{item.subtitle}</span> : null}
                      </span>
                      {item.badge ? <span style={{ background: 'var(--accent-primary)', color: 'var(--bg-surface)', borderRadius: 999, fontSize: 11, minWidth: 18, textAlign: 'center', padding: '0 5px' }}>{item.badge}</span> : null}
                    </button>
                  );
                })}
              </div>
