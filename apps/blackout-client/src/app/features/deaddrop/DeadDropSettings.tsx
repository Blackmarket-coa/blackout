...
import { useDeadDrop, useDeadDropQueueActions, useSetDeadDrop, type DeadDropConfig, type DeadDropScheduleType } from './useDeadDrop';
...
export const DeadDropSettings = ({ roomId }: { roomId: string }) => {
  ...
  return (
    <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, padding: 12, display: 'grid', gap: 10 }}>
      ...
      <label>
        Schedule type
        <select value={draft.schedule.type} onChange={(event) => onScheduleTypeChange(event.target.value as DeadDropScheduleType)} style={{ marginLeft: 8 }}>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              setDraft((prev) => ({
                ...prev,
                schedule: { ...prev.schedule, intervalMinutes: Number(event.target.value) },
              }))
            }
            style={{ marginLeft: 8 }}
          />
      ...
      <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <input type="checkbox" checked={draft.anonymize} onChange={(event) => setDraft((prev) => ({ ...prev, anonymize: event.target.checked }))} />
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) => setDraft((prev) => ({ ...prev, maxQueueSize: Number(event.target.value) }))}
          style={{ marginLeft: 8 }}
        />
      </label>
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) => setDraft((prev) => ({ ...prev, retentionHours: Number(event.target.value) }))}
          style={{ marginLeft: 8 }}
        />
      </label>
