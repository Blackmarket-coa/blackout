...
export const ProfileModal = ({ open, profile, onClose, onStartDm, onAddFriend, onBlock }: ProfileModalProps) => {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      onClick={onClose}
    >
      <div
        style={{
      ...
      >
        {/* Banner */}
        <div style={{ height: 120, background: profile.profile.banner ? undefined : 'linear-gradient(135deg, #7289DA 0%, #5865F2 100%)', position: 'relative', flexShrink: 0 }}>
          {profile.profile.banner ? (
            <img src={profile.profile.banner} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          ) : null}
          ...
          <div style={{ position: 'absolute', top: 12, right: 12, display: 'flex', gap: 8 }}>
            ...
            {onAddFriend && (
              <DcButton variant={profile.isFriend ? 'secondary' : 'primary'} onClick={() => onAddFriend(profile.userId)}>
        ...
        {/* Avatar row */}
        <div style={{ padding: '0 16px', position: 'relative', height: 52, marginBottom: 8 }}>
        ...
        {/* Body */}
        <div style={{ padding: '0 16px 16px', display: 'grid', gap: 0, color: dc.textPrimary }}>
          {/* Name block */}
          <div style={{ marginBottom: 12 }}>
            <h2 style={{ margin: '0 0 2px', fontSize: 20, fontWeight: 700, color: dc.textPrimary }}>{profile.displayName}</h2>
            <div style={{ fontSize: 13, color: dc.textMuted, fontWeight: 400 }}>
              {profile.userId}
            </div>
            {profile.profile.pronouns ? (
              <div style={{ fontSize: 13, color: dc.textSecondary, marginTop: 2 }}>{profile.profile.pronouns}</div>
            ) : null}
          </div>
          ...
          {hasBio && (
            <>
              ...
              <div
                style={{ fontSize: 14, color: dc.textSecondary, lineHeight: 1.5, marginBottom: 12 }}
                dangerouslySetInnerHTML={{ __html: toSafeHtml(profile.profile.bio) }}
              />
              <div style={{ height: 1, background: dc.divider, margin: '0 0 12px' }} />
            </>
          )}
            ...
            <>
              ...
              <div style={{ display: 'grid', gap: 6, marginBottom: 12 }}>
                {(profile.profile.connections ?? []).map((connection) => (
                  ...
                  >
                    <span style={{ color: dc.blurple, fontWeight: 600, textTransform: 'capitalize' }}>{connection.type}</span>
                    <span>{connection.label ?? connection.username ?? connection.url}</span>
                  </a>
                ))}
              </div>
              <div style={{ height: 1, background: dc.divider, margin: '0 0 12px' }} />
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                {profile.roleBadges.map((badge, i) => (
                  ...
                  >
                    <span style={{ width: 10, height: 10, borderRadius: '50%', background: getRoleColor(i), display: 'inline-block', flexShrink: 0 }} />
                    {badge}
                  </span>
                ))}
              </div>
          ...
          {profile.mutualSpaces.length > 0 && (
            <>
              <p style={dc.sectionHeader}>Mutual Servers — {profile.mutualSpaces.length}</p>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
