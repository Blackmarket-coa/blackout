    ...
    >
      ...
      <div style={{ padding: '4px 12px 12px' }}>
        <div style={{ marginBottom: 8 }}>
          <strong style={{ fontSize: 16, fontWeight: 700, color: dc.textPrimary }}>{profile.displayName}</strong>
          {profile.profile.pronouns ? (
            <div style={{ fontSize: 12, color: dc.textMuted }}>{profile.profile.pronouns}</div>
          ) : null}
          {profile.primaryRole ? (
            <div style={{ fontSize: 12, color: dc.textSecondary }}>{profile.primaryRole}</div>
          ) : null}
        </div>
        ...
        {preview.trim().length > 0 && (
          <>
            ...
            <p style={{ margin: '0 0 4px', fontSize: 12, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: dc.textSecondary }}>
              About Me
            </p>
            <div
              style={{ fontSize: 13, color: dc.textSecondary, lineHeight: 1.45 }}
              dangerouslySetInnerHTML={{ __html: sanitizeMatrixHtml(mdToHtml(preview)) }}
            />
          </>
        )}
