...
const decodePayload = async (payload: Uint8Array, passphrase: string): Promise<string | null> => {
    ...
    const expectedCrc = new DataView(payload.buffer, payload.byteOffset + 37, 4).getUint32(
        0,
        false,
    );
...
export const decodeMessageFromImage = async (
    ...
    if (payloadLength <= 0 || payloadLength > Math.floor((image.width * image.height * 3) / 8))
        return null;
