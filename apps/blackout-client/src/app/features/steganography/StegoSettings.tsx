    ...
    <section style={{ display: 'grid', gap: 12 }}>
      ...
      <div style={{ border: '1px solid var(--border-default)', borderRadius: 10, padding: 10 }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {settings.savedPassphrases.length === 0 ? <small>No saved passphrases yet.</small> : null}
            ...
            <div key={entry.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                ...
                onClick={() =>
                  setSettings((prev) => ({
                    ...prev,
                    savedPassphrases: prev.savedPassphrases.filter((item) => item.id !== entry.id),
                  }))
                }
              >
                Remove
        ...
        <div style={{ display: 'grid', gap: 6, marginTop: 10 }}>
          <input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Passphrase label" />
          <input value={passphrase} onChange={(event) => setPassphrase(event.target.value)} placeholder="Passphrase" type="password" />
          <button
            type="button"
            onClick={() => {
              ...
              setSettings((prev) => ({
                ...
                savedPassphrases: [
                  ...
                  { id: `${Date.now()}`, label: label.trim(), passphrase: passphrase.trim() },
                ],
              }));
              setLabel('');
              setPassphrase('');
