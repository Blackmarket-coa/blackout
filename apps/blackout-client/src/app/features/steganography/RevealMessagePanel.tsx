...
export const RevealMessagePanel = ({ imageUrl }: RevealMessagePanelProps) => {
  ...
  return (
    <details style={{ marginTop: 8 }}>
      <summary style={{ cursor: 'pointer', color: 'var(--accent-primary)' }}>🔐 Hidden content detected</summary>
      <div style={{ marginTop: 8, border: '1px solid var(--border-default)', borderRadius: 8, padding: 8 }}>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setError(err instanceof Error ? err.message : 'Failed to decode hidden content.');
              } finally {
                setLoading(false);
              }
            }}
        ...
        {revealed ? <pre style={{ whiteSpace: 'pre-wrap', marginBottom: 0 }}>{revealed}</pre> : null}
      </div>
    </details>
  );
};
