...
export const HideMessageDialog = ({ open, onClose, onEncoded }: HideMessageDialogProps) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 40 }} onClick={onClose}>
      <div
        style={{ width: 560, maxWidth: '92vw', margin: '10vh auto', padding: 12, borderRadius: 12, border: '1px solid var(--border-default)', background: 'var(--bg-surface)' }}
        onClick={(event) => event.stopPropagation()}
      >
        <h3 style={{ marginTop: 0 }}>Steganography Toolbox</h3>
        <p style={{ color: 'var(--text-secondary)' }}>Select an action: hide, decrypt, or create a strong passphrase for stego messages.</p>

        <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
          <button type="button" style={panelButtonStyle(activePanel === 'encode')} onClick={() => setActivePanel('encode')}>Hide Message</button>
          <button type="button" style={panelButtonStyle(activePanel === 'decrypt')} onClick={() => setActivePanel('decrypt')}>Decrypt Image</button>
          <button type="button" style={panelButtonStyle(activePanel === 'password')} onClick={() => setActivePanel('password')}>Create Password</button>
        </div>
        ...
        {activePanel === 'encode' ? (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginTop: 12 }}>
              <button type="button" onClick={() => setPassphrase(generatePassphrase(24))}>Create passphrase</button>
        ...
        {activePanel === 'decrypt' ? (
          <>
            ...
            <label style={{ display: 'grid', gap: 4, marginTop: 8 }}>
              Passphrase
              <input type="password" value={decodePassphrase} onChange={(event) => setDecodePassphrase(event.target.value)} />
            </label>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setDecodedMessage(null);
                  try {
                    const decoded = await decodeMessageFromImage(decodeImage, decodePassphrase);
                    if (!decoded) {
                      setDecodeError('No hidden message found or passphrase is incorrect.');
                      return;
                    }
                    setDecodedMessage(decoded);
                  } catch (err) {
                    setDecodeError(err instanceof Error ? err.message : 'Failed to decode hidden content.');
                  } finally {
                    setDecoding(false);
                  }
                }}
            ...
            {decodeError ? <p style={{ color: 'var(--danger)' }}>{decodeError}</p> : null}
            {decodedMessage ? <pre style={{ whiteSpace: 'pre-wrap', marginBottom: 0 }}>{decodedMessage}</pre> : null}
          </>
        ) : null}
        ...
        {activePanel === 'password' ? (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginTop: 12 }}>
              <button type="button" onClick={() => setGeneratedPassphrase(generatePassphrase(24))}>Generate new</button>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  type="button"
                  onClick={async () => {
