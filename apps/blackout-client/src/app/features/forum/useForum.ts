...
const toForumSettings = (content: Record<string, unknown> | undefined): ForumSettings => {
  ...
  const tags = Array.isArray(content.tags)
    ? content.tags
        .map((tag) => {
          ...
          if (typeof item.name !== 'string' || typeof item.color !== 'string' || typeof item.emoji !== 'string') return null;
  ...
  const defaultSort = defaultSortRaw === 'new' || defaultSortRaw === 'top' ? defaultSortRaw : 'hot';
...
const parseTagsFromMessage = (event: MatrixEvent): string[] => {
  ...
  if (Array.isArray(content['co.bmc.forum.tags'])) {
    return (content['co.bmc.forum.tags'] as unknown[]).filter((item): item is string => typeof item === 'string');
  }
  return [];
};
