...
const ThreadView = ({
  ...
  return (
    <section style={{ display: 'grid', gap: 10 }}>
      ...
      <article style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-input)', padding: 12 }}>
        <h3 style={{ margin: 0 }}>{post.title}</h3>
        <pre style={{ whiteSpace: 'pre-wrap', margin: '8px 0 0', font: 'inherit' }}>{post.body}</pre>
      </article>

      <section style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12 }}>
        ...
        <div style={{ marginTop: 8, display: 'grid', gap: 8 }}>
          {replies.map((event) => {
            ...
            return (
              <article key={event.getId() ?? String(event.getTs())} style={{ border: '1px solid var(--border-default)', borderRadius: 8, padding: 8 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{event.getSender()}</div>
        ...
        <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
          ...
          <button
            ...
            style={{ justifySelf: 'end', border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--accent-primary)', color: 'var(--bg-surface)', padding: '6px 10px' }}
          >
            Reply
          </button>
        </div>
...
export const ForumView = ({ roomId }: { roomId: string }) => {
  ...
  const filteredAndSorted = useMemo(() => {
    const now = Date.now();
    const source = activeTag ? posts.data.filter((post) => post.tags.includes(activeTag)) : posts.data;
  ...
  const selectedPost = useMemo(() => posts.data.find((post) => post.eventId === selectedPostId) ?? null, [posts.data, selectedPostId]);
  ...
  return (
    <section style={{ display: 'grid', gap: 12 }}>
      ...
      <header style={{ border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 10, display: 'grid', gap: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
          <div>
            <strong>Forum</strong>
            {forumSettings.data.guidelines ? (
              <p style={{ margin: '4px 0 0', color: 'var(--text-secondary)', fontSize: 12, whiteSpace: 'pre-wrap' }}>{forumSettings.data.guidelines}</p>
            ) : null}
          </div>
          ...
          <button
            ...
            style={{ alignSelf: 'start', border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--accent-primary)', color: 'var(--bg-surface)', padding: '6px 10px' }}
          >
            Create New Post
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(['hot', 'new', 'top'] as SortMode[]).map((mode) => (
            <button
              ...
              style={{ border: '1px solid var(--border-default)', borderRadius: 999, background: sortMode === mode ? 'var(--accent-muted)' : 'var(--bg-input)', color: 'var(--text-primary)', padding: '2px 8px', fontSize: 12 }}
            >
              {mode.toUpperCase()}
            </button>
          ))}
          ...
          {sortMode === 'top' ? (
            <select
              ...
              style={{ border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--text-primary)', padding: '4px 8px', fontSize: 12 }}
            >
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button
            ...
            style={{ border: '1px solid var(--border-default)', borderRadius: 999, background: activeTag === null ? 'var(--accent-muted)' : 'var(--bg-input)', color: 'var(--text-primary)', padding: '3px 8px', fontSize: 12 }}
          >
            All tags
          </button>
          {forumSettings.data.tags.map((tag) => (
            <button
              ...
              style={{ border: `1px solid ${tag.color}`, borderRadius: 999, background: activeTag === tag.name ? tag.color : 'transparent', color: activeTag === tag.name ? '#fff' : tag.color, padding: '3px 8px', fontSize: 12 }}
            >
              {tag.emoji} {tag.name}
            </button>
          ))}
      ...
      {selectedPost ? (
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          {!posts.loading && filteredAndSorted.length === 0 ? (
            <div style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: 16 }}>No forum posts yet.</div>
          ) : null}
        </div>
      )}
    </section>
