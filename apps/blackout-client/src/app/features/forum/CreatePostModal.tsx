...
export const CreatePostModal = ({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 50 }} onClick={onClose}>
      <div
        style={{ width: 700, maxWidth: '96vw', margin: '8vh auto', border: '1px solid var(--border-default)', borderRadius: 12, background: 'var(--bg-surface)', padding: 12 }}
        onClick={(event) => event.stopPropagation()}
      >
        <h3 style={{ marginTop: 0 }}>Create New Post</h3>

        <label style={{ display: 'grid', gap: 6, marginBottom: 10 }}>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Title (first line)</span>
          <input
            value={title}
        ...
        <div style={{ marginBottom: 10 }}>
            ...
            {tags.map((tag) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setSelectedTags((prev) => (selected ? prev.filter((item) => item !== tag.name) : [...prev, tag.name]));
                  }}
                  style={{
          ...
          {requireTag && selectedTags.length === 0 ? (
            <div style={{ marginTop: 6, color: 'var(--warning)', fontSize: 12 }}>At least one tag is required.</div>
          ) : null}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button
            type="button"
            onClick={() => void submit()}
            disabled={!title.trim() || sending || (requireTag && selectedTags.length === 0)}
