...
export const RoleEditor = ({ roomId }: { roomId: string }) => {
  ...
  const { data: roles, createRole, updateRole, deleteRole, reorderRoles, source } = useManageRoles(roomId);
  ...
  const onUploadIcon = async (file: File): Promise<string | undefined> => {
    ...
    const uploadResponse = (await client.uploadContent(file, { includeFilename: true, type: file.type })) as
      | string
      | { content_uri?: string };
    ...
    <section style={{ display: 'grid', gap: 12 }}>
      <header>
        ...
        <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 12 }}>
          ...
          {source === 'derived' ? ' No event exists yet, defaults are currently derived from power levels.' : ''}
        </p>
      </header>

      <div style={{ border: '1px solid var(--border-default)', borderRadius: 12, padding: 10, display: 'grid', gap: 10 }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
            ...
            <input
              ...
              onChange={(event) => setDraft((prev) => ({ ...prev, powerLevel: Number(event.target.value) }))}
            />
          </label>
          ...
          <fieldset style={{ border: '1px solid var(--border-default)', borderRadius: 8, padding: 8 }}>
      ...
      <div style={{ border: '1px solid var(--border-default)', borderRadius: 12, overflow: 'hidden' }}>
        {orderedRoles.map((role, index) => (
          <article
            ...
            style={{
              borderBottom: index === orderedRoles.length - 1 ? 'none' : '1px solid var(--border-default)',
              padding: 10,
              display: 'grid',
              gap: 8,
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>PL {role.powerLevel}</span>
            </div>

            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {role.permissions.map((permission) => (
                <span key={permission} style={{ fontSize: 11, border: '1px solid var(--border-default)', borderRadius: 999, padding: '2px 6px' }}>{permission}</span>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                type="color"
                value={role.color}
                onChange={(event) => void updateRole(role.position, { color: event.target.value })}
                title="Role color"
              />
              <input
                value={role.name}
                onChange={(event) => void updateRole(role.position, { name: event.target.value })}
                style={{ flex: 1, border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-surface)', color: 'var(--text-primary)', padding: '4px 8px' }}
              />
              <button
                type="button"
                onClick={() => {
            ...
            <label style={{ display: 'grid', gap: 4, fontSize: 12 }}>
              ...
              <input
                ...
                onChange={(event) => void updateRole(role.position, { powerLevel: Number(event.target.value) })}
              />
            </label>
            ...
            <fieldset style={{ border: '1px solid var(--border-default)', borderRadius: 8, padding: 8 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 4 }}>
                {AVAILABLE_PERMISSIONS.map((permission) => {
                  ...
                  return (
                    <label key={`${role.position}-${permission}`} style={{ fontSize: 12 }}>
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={(event) => {
                          ...
                          void updateRole(role.position, { permissions: next });
                        }}
                      />{' '}
                      {permission}
                    </label>
