    ...
    return {
        ...
        roomType:
            typeof record.roomType === 'string'
                ? record.roomType
                : typeof record.room_type === 'string'
                  ? record.room_type
                  : undefined,
        worldReadable:
...
const sortByChildOrder = (
    rooms: HierarchyRoom[],
    childrenState: Array<Record<string, unknown>> | undefined,
): HierarchyRoom[] => {
    ...
    childrenState.forEach((entry) => {
        ...
        const content =
            typeof entry.content === 'object' && entry.content
...
const buildTree = (
    rootChildren: HierarchyRoom[],
    all: Map<string, HierarchyRoom[]>,
): CategoryNode[] => {
            ...
            {!collapsed ? (
                <div style={{ paddingLeft: 8 }}>
                    {node.children.map((room, idx) => {
                        const restricted =
                            room.guestCanJoin === false && room.worldReadable === false;
                        const suggested = room.worldReadable && !room.numJoinedMembers;
                        const missing =
                            room.name.toLowerCase().includes('deleted') ||
                            room.name.toLowerCase().includes('tombstone');
                        ...
                        return (
                            <button
                                ...
                                onDragStart={(event: DragEvent<HTMLButtonElement>) =>
                                    event.dataTransfer.setData('text/room-id', room.roomId)
                                }
                            ...
                            >
                                ...
                                {voice ? (
                                    <span style={{ color: 'var(--accent-primary)', fontSize: 11 }}>
                                        ● {connectedUsers}
                                    </span>
                                ) : null}
                                {suggested ? (
...
export const SpaceTree = ({ spaceId }: { spaceId: string | null }) => {
    ...
    const [contextMenu, setContextMenu] = useState<{ x: number; y: number; roomId: string } | null>(
        null,
    );
    ...
    useEffect(() => {
        let mounted = true;
        if (!spaceId) {
            const generalRooms = joinedRooms
            ...
            setCachedTree([
                { id: 'general', name: 'General', children: generalRooms, subspaces: [] },
            ]);
            return;
        }
            ...
            try {
                ...
                await Promise.all(
                    rootChildren
                        ...
                        .map(async (space) => {
                            const nested = await client.getRoomHierarchy(
                                space.roomId,
                                50,
                                undefined,
                                undefined,
                ...
                const inTreeRoomIds = new Set(
                    tree.flatMap((node) => node.children.map((child) => child.roomId)),
                );
