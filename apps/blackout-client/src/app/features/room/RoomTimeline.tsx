...
const getDisplayName = (room: Room | null, userId: string): string =>
...
const isStateEvent = (event: MatrixEvent): boolean =>
    event.getType().startsWith('m.room.') && event.isState();
...
const resolveStateCopy = (event: MatrixEvent, room: Room | null): string => {
    ...
    switch (event.getType()) {
        case 'm.room.member': {
            const membership =
                typeof content.membership === 'string' ? content.membership : 'updated membership';
...
export const TextMessage = memo(({ event }: MessageBubbleProps) => {
    ...
    if (html) {
        return (
            <div
                style={styles.body}
                dangerouslySetInnerHTML={{ __html: sanitizeMatrixHtml(html) }}
            />
        );
    }
...
export const FileMessage = memo(({ event }: MessageBubbleProps) => {
    ...
    return (
        <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{ ...styles.pill, display: 'inline-block' }}
        >
            ⬇ {body}
        </a>
    );
...
export const StickerMessage = memo(({ event }: MessageBubbleProps) => {
    ...
    return (
        <img
            src={url}
            alt={getBody(event) || 'Sticker'}
            style={{ ...styles.media, border: 'none', maxWidth: 220 }}
        />
    );
});

export const NoticeMessage = memo(({ event }: MessageBubbleProps) => (
    <div style={{ ...styles.body, ...styles.notice }}>{getBody(event)}</div>
));
...
const TypingBar = ({ members }: { members: RoomMember[] }) => {
    ...
    return (
        <div style={styles.typingBar}>
            {names} {members.length === 1 ? 'is' : 'are'} typing…
        </div>
    );
};
    ...
    const getReceipts = useCallback(
        (event: MatrixEvent): ReadReceipt[] => {
            ...
            const receiptEntries = (
                room.getReceiptsForEvent(event) as Array<{ type?: string; userId: string }>
            )
