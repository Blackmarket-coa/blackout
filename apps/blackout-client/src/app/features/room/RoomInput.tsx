...
export const RoomInput = forwardRef<HTMLDivElement, RoomInputProps>(
    ({ editor, fileDropContainerRef, roomId, room }, ref) => {
        ...
        const [expressionControlVisibility, setExpressionControlVisibility] = useState(() =>
            getExpressionControlVisibility(document.body.clientWidth)
        );
        ...
        useElementSizeObserver(
            ...
            useCallback(
                (width) => setExpressionControlVisibility(getExpressionControlVisibility(width)),
                []
            )
        );
        ...
        const handleSendUpload = async (uploads: UploadSuccess[]) => {
            ...
            const contents = fulfilledPromiseSettledResult(
            ...
            (evt) => {
                if (
                    (isKeyHotkey('mod+enter', evt) ||
                        (!enterForNewline && isKeyHotkey('enter', evt))) &&
                    !isComposing(evt)
                ) {
                    evt.preventDefault();
                    submit();
        ...
        const handleGifSelect = async (mxc: string, shortcode: string, label: string) => {
            const gifUrl = mxcUrlToHttp(mx, mxc, useAuthentication);
            if (!gifUrl) return;

            const info = await getImageInfo(
                await loadImageElement(gifUrl),
                await getImageUrlBlob(gifUrl)
            );

            mx.sendMessage(roomId, {
                msgtype: MsgType.Image,
                body: label || shortcode,
                url: mxc,
                info,
            } as any);
        };
                            ...
                            <UseStateProvider initial={undefined}>
                                {(emojiBoardTab: EmojiBoardTab | undefined, setEmojiBoardTab) => (
                                    ...
                                    >
                                        ...
                                        {!expressionControlVisibility.hideStickerBtn && (
                                            <IconButton
                                                aria-pressed={
