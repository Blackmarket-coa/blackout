...
const findRelation = (event: MatrixEvent): Record<string, unknown> | null => {
  ...
  return typeof relates === 'object' && relates !== null ? (relates as Record<string, unknown>) : null;
};
...
const aggregateReactions = (
  ...
  timeline.forEach((event) => {
    ...
    if (!relation || relation.rel_type !== 'm.annotation' || relation.event_id !== targetEventId) return;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
        {common.map((emoji) => (
          <button key={emoji} type="button" onClick={() => onSelect(emoji)} style={{ border: '1px solid var(--border-default)', borderRadius: 6, background: 'var(--bg-input)' }}>
            {emoji}
          </button>
        ))}
      </div>
      {custom.length > 0 ? <div style={{ color: 'var(--text-muted)', fontSize: 11, marginBottom: 4 }}>Custom emoji</div> : null}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
        {custom.map(([key, url]) => (
          <button key={key} type="button" onClick={() => onSelect(key)} style={{ border: '1px solid var(--border-default)', borderRadius: 6, background: 'var(--bg-input)', padding: 2 }}>
...
export const Reactions = memo(({ roomId, targetEventId }: ReactionsProps) => {
  ...
  const homeserverUrl = (client as unknown as { getHomeserverUrl?: () => string }).getHomeserverUrl?.() ?? '';
  const customEmoji = useMemo(() => collectCustomEmoji(room, homeserverUrl), [homeserverUrl, room]);
  const reactions = useMemo(() => aggregateReactions(timeline, targetEventId, myUserId ?? '', customEmoji), [customEmoji, myUserId, targetEventId, timeline]);

  const visibleReactions = reactions.length > 20 ? reactions.slice(windowStart, windowStart + 20) : reactions;
    ...
    return (
      <div style={{ marginTop: 6, position: 'relative' }}>
        <button
          ...
          style={{ border: '1px dashed var(--border-default)', background: 'var(--bg-input)', color: 'var(--text-muted)', borderRadius: 999, padding: '2px 8px' }}
        >
          +
        </button>
        {pickerOpen ? <EmojiPicker customEmoji={customEmoji} recents={recent} onSelect={(emoji) => void sendReaction(emoji).then(() => setPickerOpen(false))} /> : null}
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: 6, position: 'relative' }}>
      {reactions.length > 20 ? (
        <button
          ...
          style={{ border: '1px solid var(--border-default)', borderRadius: 999, background: 'var(--bg-input)' }}
        >
          ◀
        </button>
      ) : null}
      ...
      {visibleReactions.map((reaction) => (
        <button
          ...
          style={{
            ...
            gap: 4,
            borderRadius: 999,
            border: reaction.reactedByMe ? '1px solid var(--accent-primary)' : '1px solid var(--border-default)',
            background: reaction.reactedByMe ? 'var(--accent-muted)' : 'var(--bg-input)',
            color: 'var(--text-primary)',
            padding: '2px 8px',
            fontSize: 12,
          }}
        >
          {reaction.customUrl ? <img src={reaction.customUrl} alt={reaction.key} style={{ width: 16, height: 16 }} /> : <span>{reaction.key}</span>}
      ...
      {reactions.length > 20 ? (
        <button
          type="button"
          onClick={() => setWindowStart((prev) => Math.min(prev + 10, Math.max(reactions.length - 20, 0)))}
          style={{ border: '1px solid var(--border-default)', borderRadius: 999, background: 'var(--bg-input)' }}
        >
          ▶
        </button>
      ...
      <button
        ...
        style={{ border: '1px dashed var(--border-default)', borderRadius: 999, background: 'var(--bg-input)', color: 'var(--text-primary)', padding: '2px 8px' }}
      >
        +
      </button>
