        ...
        <div key={member.userId} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span
            style={{
              ...
              borderRadius: 999,
              background:
                title === 'Online' ? 'var(--success)' : title === 'Away' ? 'var(--warning)' : 'var(--text-muted)',
            }}
          />
...
export const RightPanelContent = ({ panel, room, events, onJumpToEvent }: RightPanelContentProps) => {
  ...
  return (
    <div style={{ padding: 12, overflowY: 'auto', height: 'calc(100% - 44px)', display: 'grid', gap: 8 }}>
      ...
      {searchQuery.trim().length === 0 ? (
        <small style={{ color: 'var(--text-secondary)' }}>Type to search room messages.</small>
      ) : null}
