...
const useGroups = (
    ...
    const gifGroupItems = useMemo(() => {
        const g: GifGroupItem[] = [];
        if (tab !== EmojiBoardTab.Gif) return g;
        ...
        imagePacks.forEach((pack) => {
            ...
            g.push({
                ...
                items: pack
                    .getImages(ImageUsage.Sticker)
                    .filter((item) => item.info?.mimetype === 'image/gif')
                    .sort((a, b) => a.shortcode.localeCompare(b.shortcode)),
            });
        });
...
const useItemRenderer = (tab: EmojiBoardTab) => {
    ...
    const renderItem = (emoji: IEmoji | PackImageReader, index: number) => {
        ...
        if (tab === EmojiBoardTab.Sticker || tab === EmojiBoardTab.Gif) {
            return (
                <StickerItem
                    ...
                    image={emoji}
                    type={tab === EmojiBoardTab.Gif ? EmojiType.Gif : EmojiType.Sticker}
                />
            );
        }
        return (
            ...
            {packs.length > 0 && (
                <SidebarStack>
                    ...
                    {packs.map((pack) => {
                        let label = pack.meta.name;
                        if (!label)
                            label = isUserId(pack.id) ? 'Personal Pack' : mx.getRoom(pack.id)?.name;
...
export function EmojiBoard({
    ...
    const [emojiGroupItems, stickerGroupItems, gifGroupItems] = useGroups(tab, imagePacks);
    const groups = emojiTab
        ? emojiGroupItems
    ...
    const handleGroupItemClick: MouseEventHandler = (evt) => {
        ...
        if (emojiInfo.type === EmojiType.Gif) {
            onGifSelect?.(emojiInfo.data, emojiInfo.shortcode, emojiInfo.label);
        }
