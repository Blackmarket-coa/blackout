...
export const getInfo = (event: MatrixEvent): Record<string, unknown> => {
  const content = event.getContent<Record<string, unknown>>();
  return typeof content.info === 'object' && content.info !== null ? (content.info as Record<string, unknown>) : {};
};
  ...
  useEffect(() => {
    ...
    const resolveSource = async () => {
      ...
      try {
        ...
        const decrypt = (client as unknown as { decryptMedia?: (data: ArrayBuffer, file: Record<string, unknown>) => Promise<ArrayBuffer> }).decryptMedia;
        if (!decrypt) {
...
export const baseStyles = {
  ...
  skeleton: {
    background: 'linear-gradient(90deg, var(--bg-input), var(--bg-surface-hover), var(--bg-input))',
