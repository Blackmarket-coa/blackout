...
import { baseStyles, formatBytes, getInfo, useInViewport, useResolvedMediaSource } from './mediaShared';
...
export const FileMessage = ({ event, enablePdfPreview = true }: FileMessageProps) => {
  ...
  return (
    <div ref={ref} style={{ ...baseStyles.surface, padding: 10, width: 'min(440px, 100%)' }}>
      {!inView || loading ? <div style={{ ...baseStyles.skeleton, minHeight: 72, borderRadius: 8 }} /> : null}
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ color: 'var(--text-primary)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis' }}>{fileName}</div>
              <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                {mime ?? 'unknown'} · {formatBytes(typeof info.size === 'number' ? info.size : undefined)}
              </div>
            </div>
            <a
              href={src}
              ...
              rel="noreferrer"
              download={fileName}
              style={{ border: '1px solid var(--border-default)', borderRadius: 8, padding: '4px 8px', color: 'var(--text-primary)' }}
            >
              Download
            </a>
          </div>
          ...
          {enablePdfPreview && isPdf ? (
            <iframe title={fileName} src={src} style={{ marginTop: 8, width: '100%', height: 220, border: '1px solid var(--border-default)', borderRadius: 8 }} />
          ) : null}
        </>
      ) : null}
    </div>
