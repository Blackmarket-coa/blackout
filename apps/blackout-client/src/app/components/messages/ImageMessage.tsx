...
export const ImageMessage = ({ event }: ImageMessageProps) => {
    ...
    const blurhash =
        typeof info['xyz.amorgan.blurhash'] === 'string' ? info['xyz.amorgan.blurhash'] : null;
        ...
        <div ref={ref} style={{ width: 'min(460px, 100%)' }}>
            ...
            {inView && loading ? (
                <div style={placeholderStyle}>
                    {blurhash ? 'Blurhash placeholder' : 'Loading image…'}
                </div>
            ) : null}
            {inView && error ? (
                <div style={baseStyles.error}>Image failed to load: {error}</div>
            ) : null}
