...
export const AudioMessage = ({ event }: AudioMessageProps) => {
  ...
  const waveform = useMemo(() => Array.from({ length: 40 }, (_, i) => 20 + Math.abs(Math.sin(i / 3)) * 65), []);

  return (
    <div ref={ref} style={{ ...baseStyles.surface, padding: 10, width: 'min(420px, 100%)' }}>
      {!inView || loading ? <div style={{ ...baseStyles.skeleton, minHeight: 54, borderRadius: 8 }} /> : null}
        ...
        <>
              ...
              {waveform.map((height, idx) => (
                <span
                  ...
                  style={{
                    ...
                    borderRadius: 2,
                    background: idx / waveform.length < position ? 'var(--accent-primary)' : 'var(--accent-muted)',
                  }}
                />
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
            ...
            <input
              ...
              style={{ flex: 1 }}
            />
            <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>{formatTime((audioRef.current?.currentTime ?? position * durationSec) || 0)} / {formatTime(durationSec)}</span>
          </div>
        </>
      ) : null}
    </div>
