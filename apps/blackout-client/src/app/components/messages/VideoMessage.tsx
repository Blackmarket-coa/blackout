...
export const VideoMessage = ({ event }: VideoMessageProps) => {
  ...
  const poster = useMemo(() => (typeof info.thumbnail_url === 'string' ? info.thumbnail_url : undefined), [info.thumbnail_url]);

  return (
    <div ref={ref} style={{ ...baseStyles.surface, padding: 8, width: 'min(540px, 100%)' }}>
      {!inView || loading ? <div style={{ ...baseStyles.skeleton, borderRadius: 8, minHeight: 180 }} /> : null}
      ...
      {inView && src ? (
        <>
          ...
          {encrypted ? <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>Encrypted video</div> : null}
        </>
      ) : null}
    </div>
  );
