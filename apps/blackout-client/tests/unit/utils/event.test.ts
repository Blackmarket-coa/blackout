...
describe('event utils', () => {
    ...
    it('getEventFormattedBody extracts formatted_body html', () => {
        expect(getEventFormattedBody(mockEvent({ content: { formatted_body: '<b>hi</b>' } }))).toBe(
            '<b>hi</b>',
        );
    });
    ...
    it('getEditedContent extracts m.new_content', () => {
        ...
        expect(getEditedContent(mockEvent({ content: { 'm.new_content': newContent } }))).toEqual(
            newContent,
        );
    });
    ...
    it('getEditedContent returns null when m.new_content is absent', () => {
        ...
        expect(
            getEditedContent(mockEvent({ content: { 'm.new_content': 'not-an-object' } })),
        ).toBeNull();
    });
    ...
    it('isNotificationEvent returns false for redacted or edited events', () => {
        expect(isNotificationEvent(mockEvent({ type: 'm.room.message', isRedacted: true }))).toBe(
            false,
        );
        expect(
            isNotificationEvent(
                mockEvent({ type: 'm.room.message', relation: { rel_type: 'm.replace' } }),
            ),
        ).toBe(false);
    });
});
