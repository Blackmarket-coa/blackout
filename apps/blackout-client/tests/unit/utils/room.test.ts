        ...
        currentState: {
            getStateEvents: (eventType: string, stateKey?: string) => {
                ...
                if (eventType === 'm.room.join_rules') {
                    return overrides.joinRule
                        ? { getContent: () => ({ join_rule: overrides.joinRule }) }
                        : null;
                }
                if (eventType === 'm.room.power_levels') {
                    return overrides.powerLevels
                        ? { getContent: () => overrides.powerLevels }
                        : null;
                }
                return null;
            },
...
describe('room utils', () => {
    it('getRoomName returns display name fallback', () => {
        ...
        expect(getRoomName(mockRoom({ name: '', roomId: '!abc:example.org' }))).toBe(
            '!abc:example.org',
        );
    });
    ...
    it('getRoomAvatar resolves avatar URL from mxc', () => {
        expect(
            getRoomAvatar(
                mockRoom({ avatarUrl: 'https://matrix.example.org/media/avatar.png' }),
                mockClient(),
            ),
        ).toBe('https://matrix.example.org/media/avatar.png');
