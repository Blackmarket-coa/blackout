...
import { SpaceSettings } from '../../../../src/app/features/space-settings/SpaceSettings';
import {
    renderDialog,
...
describe('SpaceSettings reliability (content-component contract)', () => {
    beforeEach(() => {
    ...
    it('mobile close affordance fires requestClose', async () => {
        ...
        const mounted = await renderDialog(<SpaceSettings requestClose={requestClose} />, {
            store: createStore(),
        });
        ...
        try {
            for (let i = 0; i < 5; i += 1) {
                const mounted = await renderDialog(
                    <SpaceSettings requestClose={() => undefined} />,
                    { store: createStore() },
                );
                mounted.unmount();
            }
