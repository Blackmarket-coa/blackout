...
import type { ProposalModel, VoteModel } from '../../../../src/app/features/governance/useProposals';
...
let voteByProposal: Record<string, VoteModel[]> = {
  'proposal-a': [
    { eventId: 'vote-old', proposalEventId: 'proposal-a', voterId: '@me:example.org', choice: 'no', timestamp: 100 },
    { eventId: 'vote-new', proposalEventId: 'proposal-a', voterId: '@me:example.org', choice: 'yes', timestamp: 101 },
  ],
  'proposal-b': [],
};

vi.mock('../../../../src/app/features/governance/ProposalCard', () => ({
  ProposalCard: ({ proposal, onOpen }: { proposal: ProposalModel; onOpen: (proposalId: string) => void }) => (
...
vi.mock('../../../../src/app/features/governance/ProposalDetail', () => ({
  ProposalDetail: ({ proposalId }: { proposalId: string }) => <div data-testid="proposal-detail">detail:{proposalId}</div>,
}));
...
describe('GovernanceDashboard sections', () => {
  beforeEach(() => {
    ...
    voteByProposal = {
      'proposal-a': [{ eventId: 'vote-new', proposalEventId: 'proposal-a', voterId: '@me:example.org', choice: 'yes', timestamp: 101 }],
      'proposal-b': [],
    };
  });
  ...
  it('renders Active and Past sections via tabs', () => {
    ...
    const pastTab = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Past') as HTMLButtonElement;
    act(() => pastTab.click());
  ...
  it('renders Create and Results sections', () => {
    ...
    const createTab = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Create') as HTMLButtonElement;
    act(() => createTab.click());
    expect(container.querySelector('[data-testid="proposal-creator"]')).toBeTruthy();

    const resultsTab = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Results') as HTMLButtonElement;
  ...
  it('renders My Votes using latest vote per proposal and opens ProposalDetail', () => {
    ...
    const myVotesTab = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'My Votes') as HTMLButtonElement;
    ...
    const openButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'View / Change Vote') as HTMLButtonElement;
    act(() => openButton.click());

    expect(container.querySelector('[data-testid="proposal-detail"]')?.textContent).toContain('detail:proposal-a');
  });
});
