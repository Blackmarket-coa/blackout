...
describe('normalizeMarketplaceEventContent', () => {
    ...
    it('detects ledger, dispute, cycle, logistics, flash_sale blocks', () => {
        const cases: Array<[string, Record<string, unknown>, NormalizedMarketplaceEvent['kind']]> = [
            [FBM_LEDGER_EVENT_TYPE, { schemaVersion: 1, kind: 'escrow_released', vendorId: 'v', amountMinorUnits: 4200, currency: 'USD', ledgerTxId: 'tx', occurredAt: 'x' }, 'ledger'],
            [FBM_DISPUTE_EVENT_TYPE, { schemaVersion: 1, disputeId: 'd', vendorId: 'v', status: 'open', occurredAt: 'x' }, 'dispute'],
            [FBM_CYCLE_EVENT_TYPE, { schemaVersion: 1, kind: 'open', vendorId: 'v', cycleId: 'c', name: 'Spring', occurredAt: 'x' }, 'cycle'],
            [FBM_LOGISTICS_EVENT_TYPE, { schemaVersion: 1, kind: 'delivered', vendorId: 'v', orderId: 'o', occurredAt: 'x' }, 'logistics'],
            [FBM_FLASH_SALE_EVENT_TYPE, { schemaVersion: 1, vendorId: 'v', saleId: 's', name: 'Sale', discount: '30%', durationSeconds: 900, endsAt: 'x', occurredAt: 'x' }, 'flash_sale'],
        ];
...
describe('MarketplaceEventCard', () => {
    ...
    it('renders a ledger card with the labelled amount', () => {
        const text = render({
            kind: 'ledger',
            data: { schemaVersion: 1, kind: 'escrow_released', vendorId: 'v', orderId: 'ord_1', amountMinorUnits: 4200, currency: 'USD', occurredAt: 'x' },
        });
    ...
    it('renders a flash-sale card with discount', () => {
        const text = render({
            kind: 'flash_sale',
            data: { schemaVersion: 1, vendorId: 'v', saleId: 's', name: 'Tomato blowout', discount: '30%', durationSeconds: 900, endsAt: 'soon', occurredAt: 'x' },
        });
