  ...
  ({
    ...
    currentState: {
      getStateEvents: (eventType: string) => {
        ...
        if (eventType === 'm.room.pinned_events') return { getContent: () => ({ pinned: [] }) };
        return undefined;
      },
    },
  }) as unknown as Room;
...
describe('Draupnir moderation navigation', () => {
  ...
  it('navigates from ClientLayout moderation entry to Draupnir dashboard route', async () => {
    ...
    const router = createMemoryRouter(
      [
        { path: '/', element: <Provider store={store}><ClientLayout /></Provider> },
        { path: '/moderation/draupnir', element: <Provider store={store}><DraupnirRoutePage /></Provider> },
      ],
      { initialEntries: ['/'] },
    );
    ...
    const moderationLink = Array.from(container.querySelectorAll('a')).find((link) => link.textContent === 'Moderation') as HTMLAnchorElement;
    expect(moderationLink).toBeTruthy();

    await act(async () => {
      moderationLink.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, button: 0 }));
      await Promise.resolve();
    });
