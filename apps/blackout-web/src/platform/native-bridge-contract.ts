...
export function dispatchNativeBridgeEvent(event: NativeBridgeEvent): void {
  globalThis.dispatchEvent(new CustomEvent<NativeBridgeEvent>(NATIVE_BRIDGE_EVENT_NAME, { detail: event }));
}

export function listenForNativeBridgeEvents(handler: (event: NativeBridgeEvent) => void): () => void {
