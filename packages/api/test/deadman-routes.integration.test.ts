...
test('only the owner can check in or cancel a switch', async () => {
  ...
  assert.equal(armed.status, 201);
  const switchId = ((await armed.json()) as { payload: { switchId: string } }).payload
    .switchId;

  const stolenCheckIn = await app.request(
    `/v1/deadman/switches/${switchId}/check-in`,
    {
      method: 'POST',
  ...
  const stolenCancel = await app.request(
    `/v1/deadman/switches/${switchId}/cancel`,
    {
      method: 'POST',
      headers: intruderHeaders,
    }
...
test('process-overdue advances grace and triggered transitions', async () => {
  ...
  const armed = await app.request('/v1/deadman/switches', {
    ...
    headers,
    body: JSON.stringify(
      armBody({ checkInIntervalSeconds: 60, gracePeriodSeconds: 60 })
    ),
  });
  assert.equal(armed.status, 201);
  const switchId = ((await armed.json()) as { payload: { switchId: string } }).payload
  ...
  const triggeredEntry = triggeredBody.processed.find(
    (entry) => entry.switchId === switchId
  );
...
test('switches list scopes results by owner vs recipient', async () => {
  ...
  assert.ok(
    recipientList.switches.some((entry) => entry.recipients.includes(recipient))
  );
