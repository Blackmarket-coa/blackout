...
globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    if (url.includes('/_synapse/admin/v1/registration_tokens/new')) {
        return new Response(
            JSON.stringify({ token: `synapse-${randomUUID().slice(0, 8)}`, uses_allowed: null }),
            {
...
test('follows: follow, list, idempotency, self-follow and unfollow', async () => {
    ...
    // `overlaps` reports whether the two circles now point both ways — false here
    // because b has not followed a back.
    assert.deepEqual(await follow.json(), {
        ok: true,
        following: true,
        created: true,
        overlaps: false,
    ...
    const following = await app.request('/v1/follows/following', {
        headers: bearer(a.id, a.username),
    });
    ...
    const followers = await app.request('/v1/follows/followers', {
        headers: bearer(b.id, b.username),
    });
...
test('personal link is unlimited, never expires, and is stable across calls', async () => {
    ...
    const first = await app.request('/v1/invitations/personal', {
    ...
    const second = await app.request('/v1/invitations/personal', {
        headers: bearer(owner.id, owner.username),
    });
...
test('signing up via a personal link follows the owner and the link never exhausts', async () => {
    ...
    const personal = await app.request('/v1/invitations/personal', {
        headers: bearer(owner.id, owner.username),
    });
    ...
    const followerIds = (
        (await followers.json()) as { followers: { userId: string }[] }
    ).followers.map((u) => u.userId);
...
test('GET /v1/i/:token renders OG tags and redirects carrying the registration token', async () => {
    ...
    assert.match(
        html,
        /#registrationToken=/,
        'redirect must include the registration token fragment'
    );
