...
async function entitleUser(userId: string) {
    // W1b: the Lago/Stripe webhook routes are retired — settled FBM purchases
    // drive Canopy state through applySubscriptionWebhookEvent (the same seam
    // the marketplace webhook dispatcher calls).
...
test('LS-MSG-01: room message send + list round-trip preserves order', async () => {
    ...
    assert.ok(
        messages.some((m) => m.id === sendBody.message.id),
        'sent message not in list'
    );
});
...
test('LS-MSG-02: DM-shaped channel send + list works for both participants', async () => {
    ...
    assert.ok(
        body.some((m) => m.userId === a.userId),
        'A message missing'
    );
    assert.ok(
        body.some((m) => m.userId === b.userId),
        'B message missing'
    );
});
...
test('LS-CALL-01: entitled user joining a voice room receives a LiveKit token', async () => {
    ...
    const body = (await res.json()) as {
        livekit?: { token?: string; url?: string; expiresAt?: string };
    };
