...
function createStreamingRouter() {
            ...
            return c.json(
                {
                    ...
                    message: 'No managed stream key found for creator',
                },
                404
            );
    ...
    streaming.put('/streams/:streamId/metadata', async (c) => {
        ...
        const nextDenId = parsed.denId === undefined ? stream.denId : parsed.denId ?? undefined;
    ...
    streaming.get('/vod-files/:sessionId', (c) => {
        ...
        if (match && (match[1] || match[2])) {
            ...
            const end =
                match[1] && match[2] ? Math.min(Number.parseInt(match[2], 10), size - 1) : size - 1;
    ...
    streaming.post('/streams/:streamId/sessions', async (c) => {
        ...
        const session = db.createStreamSession({
            ...
            replayPointer: parsed.replayPointer,
        });
        ...
        void dispatchOutboundEvent({
            ...
            log.warn('streaming_outbound_dispatch_threw', {
                type: 'livestream.started',
                error: String(err),
            })
        );
        ...
        // Auto-record the session to a local mp4 (BLACKOUT_VOD_RECORDING=1).
        // Best-effort like the fan-out: a recorder failure never blocks go-live.
        try {
            const recording = startVodRecording(session.id, streamId);
            if (recording.started) {
                log.info('streaming_vod_recording_started', { streamId, sessionId: session.id });
            }
        } catch (err) {
            log.warn('streaming_vod_recording_start_threw', { streamId, error: String(err) });
        }
    ...
    streaming.patch('/streams/:streamId/sessions/:sessionId', async (c) => {
        ...
        if (stream) {
            ...
            // Gracefully stop the session recorder; when ffmpeg finalizes the mp4
            // the worker attaches the replayPointer to this session (unless the
            // creator already supplied their own pointer above).
            try {
                stopVodRecording(session.id);
            } catch (err) {
                log.warn('streaming_vod_recording_stop_threw', {
                    streamId: session.streamId,
                    error: String(err),
                });
            }
    ...
    streaming.get('/streams/:streamId/moderation', (c) => {
        ...
        if (ownerDenied) return ownerDenied;
        return c.json(
            db.getStreamModeration(streamId) ?? {
                streamId,
                slowModeSeconds: 0,
                bannedUserIds: [],
                keywordFilters: [],
    ...
    streaming.get('/streams/:streamId/goal', (c) => {
        ...
        if (!Number.isFinite(targetCents) || !Number.isInteger(targetCents) || targetCents < 0) {
            return c.json(
                { code: 'invalid_target', message: 'targetCents must be a non-negative integer' },
                400
            );
        }
