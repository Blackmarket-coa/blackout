...
function createDeadmanRouter() {
  ...
  deadman.post('/switches', async (c) => {
    ...
    if (ownerId !== subject) {
      return c.json(
        { code: 'forbidden', message: 'A switch can only be armed for the calling subject' },
        403
      );
    }
  ...
  deadman.post('/switches/:id/check-in', (c) => {
    ...
    if (existing.status === 'triggered') {
      return c.json(
        { code: 'switch_triggered', message: 'Switch already triggered; payload has shipped' },
        409
      );
    }
    if (existing.status === 'cancelled') {
      return c.json(
        { code: 'switch_cancelled', message: 'Switch has been cancelled' },
        409
      );
    }
  ...
  deadman.post('/switches/:id/cancel', (c) => {
    ...
    if (existing.ownerId !== subject) {
      return c.json(
        { code: 'forbidden', message: 'Only the switch owner may cancel' },
        403
      );
    }
    if (existing.status === 'triggered') {
  ...
  deadman.get('/switches/:id', (c) => {
    ...
    const visible =
      existing.ownerId === subject || existing.recipients.includes(subject);
    if (!visible) {
      return c.json(
