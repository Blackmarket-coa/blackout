...
export function canAccessDomain(c: Context, domain: 'governance' | 'forum' | 'deaddrop' | 'moderation' | 'streaming' | 'discovery' | 'profile' | 'stego', action: 'read' | 'write'): boolean {
  const claims = getClaims(c);
...
export function requireDomainCapability(c: Context, domain: 'governance' | 'forum' | 'deaddrop' | 'moderation' | 'streaming' | 'discovery' | 'profile' | 'stego', action: 'read' | 'write'): Response | null {
