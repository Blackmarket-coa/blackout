...
export class SynapseClient implements MatrixAdminClient {
  ...
  async provisionBurner(label: string) {
    ...
    try {
      response = await fetchWithTimeout(
        `${hs}/_synapse/admin/v2/users/${encodeURIComponent(userId)}`,
        {
          method: 'PUT',
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  async purgeRoom(roomId: string, opts: { block?: boolean; purge?: boolean } = {}) {
    ...
    try {
      response = await fetchWithTimeout(
        `${hs}/_synapse/admin/v2/rooms/${encodeURIComponent(roomId)}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
