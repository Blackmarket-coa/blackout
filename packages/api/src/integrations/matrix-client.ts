...
const homeserver = () => process.env.MATRIX_HOMESERVER ?? process.env.MATRIX_HOMESERVER_URL;
...
/**
 * The only Megolm algorithm Matrix defines for room encryption. Named here so
 * every call site agrees and a typo in one of them cannot silently produce a
 * room the clients refuse to encrypt.
 */
...
export const matrixClient = {
    ...
    async registerUser(username: string, password: string) {
        ...
        const response = await fetch(
            `${hs}/_synapse/admin/v2/users/@${encodeURIComponent(username)}:${homeserverDomain()}`,
            {
                method: 'PUT',
                headers: {
    ...
    async resolveRoomAlias(alias: string) {
        ...
        if (!response.ok) {
            return {
                ok: false as const,
                status: response.status,
                reason: 'alias_not_found' as const,
            };
        }
     ...
     *
     * `encrypted` is REQUIRED and has no default on purpose. Before the
     * 2026-08-10 encryption audit this method could not express `initial_state`
     * at all, so every server-provisioned room was silently born in plaintext
     * (see docs/audits/2026-08-10-encryption-audit.md). Making the flag
     * mandatory means the compiler — not a reviewer's memory — forces each call
     * site to state its intent, and a new caller cannot inherit a silent default.
     *
     * Pass `false` only for rooms this bot must post readable content into. The
     * bot is a plain HTTP client with no Olm/Megolm implementation, so it cannot
     * send into an encrypted room; those rooms are disclosed as server-visible in
     * TRUST.md rather than being quietly encrypted and broken.
     */
    async createRoom(input: {
        ...
        /**
         * Whether to enable Megolm at creation. Set via `initial_state` rather than
         * a follow-up `sendStateEvent` so there is no window in which the room
         * exists unencrypted, and so a failure fails the create instead of leaving
         * a plaintext room behind.
         */
        encrypted: boolean;
        ...
        /** Extra `initial_state` events, appended after the encryption event. */
        initialState?: Array<{
            type: string;
            state_key?: string;
        ...
        if (!response.ok) {
            ...
            return {
                ok: false as const,
                status: response.status,
                reason: 'create_rejected' as const,
                detail,
            };
        }
        const json = (await response.json()) as { room_id?: string };
    ...
    async uploadContent(bytes: Uint8Array, contentType: string, filename?: string) {
        ...
        if (!response.ok) {
            ...
            return {
                ok: false as const,
                status: response.status,
                reason: 'upload_rejected' as const,
                detail,
            };
        }
        ...
        if (!json.content_uri) {
            return {
                ok: false as const,
                status: response.status,
                reason: 'no_content_uri' as const,
            };
        }
    ...
    async mintRegistrationToken(input: {
        ...
        const response = await fetch(`${hs}/_synapse/admin/v1/registration_tokens/new`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
        ...
        if (!json.token) {
            return {
                ok: false as const,
                status: response.status,
                reason: 'synapse_no_token' as const,
            };
        }
        return {
            ok: true as const,
    ...
    async adminJoinUserToRoom(roomId: string, userId: string) {
        ...
        let response: Response;
        try {
            response = await fetch(`${hs}/_synapse/admin/v1/join/${encodeURIComponent(roomId)}`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    async deactivateUser(userId: string, erase = false) {
        ...
        try {
            response = await fetch(
                `${hs}/_synapse/admin/v1/deactivate/${encodeURIComponent(userId)}`,
                {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    async sendStateEvent(
        ...
        const response = await fetch(
            `${hs}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/state/${encodeURIComponent(
                eventType
    ...
    async sendEvent(
        ...
        const response = await fetch(
            `${hs}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/send/${encodeURIComponent(
                eventType
