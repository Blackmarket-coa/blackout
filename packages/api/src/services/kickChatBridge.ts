...
const validateInput = (input: CreateBridgeInput): { ok: true } | { ok: false; reason: string } => {
    ...
    if (!KICK_CHATROOM_RE.test(ch)) {
        return {
            ok: false,
            reason: 'kickChatroomId must be a positive integer (no leading zeros)',
        };
    }
...
export const resumeAllBridges = (options: BridgeServiceOptions = {}): { resumed: number } => {
    let resumed = 0;
