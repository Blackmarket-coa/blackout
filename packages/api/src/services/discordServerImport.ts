...
): Promise<ListGuildsOutcome> => listImportableGuilds(userId, deps);
...
export const applyImport = async (
    ...
    if (guildMapping) {
        ...
        const res = await matrix.createRoom({
            ...
            // Spaces carry only membership/hierarchy state, never messages, so
            // encrypting one protects nothing. The imported channels below are what
            // hold conversation, and those are encrypted.
            encrypted: false,
        });
        if (!res.ok) {
            db.updateDiscordServerImport(importId, {
                status: 'failed',
                reason: res.reason ?? 'matrix_failed',
            });
    ...
    for (const channel of snapshot.channels) {
        ...
        const res = await matrix.createRoom({
            ...
            preset: 'private_chat',
            visibility: 'private',
            // Imported channels are private conversations from here on. Discord
            // history is not backfilled, so there is no readable past to lose.
            encrypted: true,
        });
        if (!res.ok) {
