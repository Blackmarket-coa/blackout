 ...
 */
const aadFor = (subscriptionId: string): string => `outbound_event_webhook|${subscriptionId}`;
...
/**
 * Reject targets that resolve to loopback, private, link-local, CGNAT, or
 * cloud-metadata space. Covers IPv4 literals (RFC1918, 127/8, 169.254/16,
 * 100.64/10, 0/8), IPv6 literals (::1, ::, fe80::/10 link-local, fc00::/7 ULA,
 * IPv4-mapped), and obvious internal hostnames. This is a static host-string
 * check; it does NOT defeat DNS-rebinding (a name that resolves to a private
 * address at fetch time) — that residual is tracked separately.
 */
...
const validateRegister = (input: RegisterInput): { ok: true } | { ok: false; reason: string } => {
    ...
    // Block localhost / private / link-local / metadata targets to defang SSRF.
    // The simulcast destinations service has the same posture.
...
export interface BlackoutEvent {
    ...
    occurredAt?: string;
    /**
     * Optional stable idempotency key for this logical event. When set, every
     * delivery attempt (including retries) reuses a deterministic delivery id
     * derived from it, so the receiver can dedupe. When absent, each attempt gets
     * a fresh random delivery id (legacy behavior).
     */
    dedupeKey?: string;
}
...
const dispatchOnce = async ({
    ...
    const deliveryId = event.dedupeKey
        ? `${record.id}.${event.type}.${event.dedupeKey}`
        : `${record.id}.${event.type}.${attempt}.${randomUUID()}`;
    ...
    try {
        ...
        return {
            ok: false,
            reason: String((err as Error)?.name === 'AbortError' ? 'timeout' : err),
        };
    } finally {
        clearTimeout(timer);
    }
...
export const dispatchEvent = async (
    ...
    const candidates = db
        ...
        .filter(
            (r) => r.blackoutUserId === event.blackoutUserId && matchesEventType(r, event.type)
        );
