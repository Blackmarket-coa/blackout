...
const readSecret = (): { secret: string | undefined; provider: BillingProvider } => {
  const provider = (process.env.BILLING_WEBHOOK_PROVIDER ?? 'lago').toLowerCase() as BillingProvider;
  const secret =
    process.env.BILLING_WEBHOOK_SECRET ??
    (provider === 'stripe' ? process.env.STRIPE_WEBHOOK_SECRET : process.env.LAGO_WEBHOOK_SECRET);
