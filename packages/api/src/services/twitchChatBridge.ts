...
import { subscribeToBridgeEvents, unsubscribeBridgeEvents } from './twitchEventSubManager';
...
const validateInput = (input: CreateBridgeInput): { ok: true } | { ok: false; reason: string } => {
    ...
    if (!/^[a-zA-Z0-9_]{1,25}$/.test(ch))
        return {
            ok: false,
            reason: 'twitchChannel must be a Twitch login (1-25 chars, [A-Za-z0-9_])',
        };
    if (!input.matrixRoomId?.trim()) return { ok: false, reason: 'matrixRoomId is required' };
    if (!MATRIX_ROOM_RE.test(input.matrixRoomId.trim())) {
        return {
            ok: false,
            reason: 'matrixRoomId must look like "!roomid:server" or "#alias:server"',
        };
    }
    return { ok: true };
};
        ...
        publishChatMessage(
            {
                ...
                channelKey: `#${record.twitchChannel.toLowerCase()}`,
            },
            {
                source: 'twitch',
...
export const resumeAllBridges = async (
    ...
    for (const record of db.listActiveTwitchChatBridges()) {
        ...
        try {
            ...
            skipped += 1;
            log.warn('twitch_chat_bridge_resume_failed', {
                bridgeId: record.id,
                error: String(err),
            });
        }
