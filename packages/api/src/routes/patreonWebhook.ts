 ...
 */
export const defaultPatreonForwarder = (event: NormalizedPatreonEvent): void => {
    const linkedAccountsForCampaign = [...db.linkedAccounts.values()].find(
        (row) => row.provider === 'patreon' && row.providerUserId === event.campaignUserId
    );
...
export const buildPatreonWebhookRoute = (options: PatreonWebhookRouteOptions = {}): Hono => {
    const router = new Hono();
    // Cap the raw body BEFORE buffering it for HMAC-MD5 verification (M5).
    ...
    router.post('/', async (c) => {
        ...
        if (!secret) {
            ...
            return c.json(
                {
                    code: 'patreon_webhook_misconfigured',
                    message: 'PATREON_WEBHOOK_SECRET is not set.',
                },
                503
            );
        }
