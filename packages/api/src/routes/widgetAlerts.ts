...
import {
    publish as publishWidgetAlert,
    subscribe as subscribeWidgetBus,
} from '../services/widgetBus';
...
/**
 * Per-token concurrency cap for the SSE stream. The widget secret travels in
 * the `?token=` query string (OBS browser sources can't set an Authorization
 * header), so it is more exposure-prone than a normal bearer; capping
 * concurrent streams per token bounds the blast radius of a leaked secret and
 * plain resource abuse. In-process only — a soft protection sufficient for
 * this abuse class (a determined attacker with the secret can already act as
 * the widget; this stops trivial fan-out from a leaked URL).
 */
const MAX_CONCURRENT_STREAMS_PER_TOKEN = 5;
const activeStreamsByToken = new Map<string, number>();
 ...
 * linked Twitch.
 */
const buildSyntheticEvent = (body: z.infer<typeof testAlertSchema>): NormalizedTwitchEvent => {
...
widgetAlerts.delete('/tokens/:id', (c) => {
    ...
    switch (out.kind) {
        ...
        case 'already_revoked':
            return c.json({ code: 'already_revoked', message: 'Token is already revoked.' }, 409);
        default: {
...
widgetAlerts.get('/stream', (c) => {
    ...
    const activeForToken = activeStreamsByToken.get(verified.id) ?? 0;
    if (activeForToken >= MAX_CONCURRENT_STREAMS_PER_TOKEN) {
        return c.json(
            {
                code: 'too_many_streams',
                message: 'Too many concurrent streams for this widget token.',
            },
            429
        );
    }
    activeStreamsByToken.set(verified.id, activeForToken + 1);

    // The secret is in the URL for OBS's sake; ensure it can't leak onward as a
    // Referer to any content an overlay page subsequently loads.
    c.header('Referrer-Policy', 'no-referrer');

    // Decrement the per-token counter exactly once when this stream ends
    // (normal completion or abort).
    let released = false;
    const release = () => {
        if (released) return;
        released = true;
        const remaining = (activeStreamsByToken.get(verified.id) ?? 1) - 1;
        if (remaining <= 0) activeStreamsByToken.delete(verified.id);
        else activeStreamsByToken.set(verified.id, remaining);
    };
