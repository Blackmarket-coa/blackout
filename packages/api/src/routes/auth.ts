...
const autoJoinWelcomeRoom = async (username: string): Promise<void> => {
    ...
    const alias =
        process.env.WELCOME_MATRIX_ROOM_ALIAS?.trim() || `#welcome:${matrixHomeserverDomain()}`;
    ...
    if (!roomId) {
        ...
        if (!resolved.ok || !resolved.roomId) {
            if (resolved.reason !== 'matrix_not_configured') {
                log.warn('register.welcome_room.resolve_failed', {
                    alias,
            ...
            return c.json(
                {
                    ...
                    message: 'Invitation token is not usable',
                    reason: outcome.kind,
                },
                400
            );
    ...
    return c.json(
        {
            ...
            message: 'Could not allocate an account, please try again.',
        },
        503
    );
});
...
auth.post('/email/verify/request', async (c) => {
    ...
    switch (issued.kind) {
        case 'ok': {
            ...
            if (!dispatch.ok) {
                return c.json(
                    { code: 'mail_send_failed', message: 'Could not send verification email' },
                    502
                );
            }
    ...
    switch (outcome.kind) {
        case 'ok':
            return c.json({
                ...
                emailVerifiedAt: outcome.user.emailVerifiedAt,
            });
        case 'expired':
            return c.json({ code: 'token_expired', message: 'Verification token expired' }, 410);
        case 'consumed':
            return c.json(
                { code: 'token_consumed', message: 'Verification token already used' },
                410
            );
        case 'revoked':
        ...
        case 'email_changed':
            return c.json(
                {
                    code: 'email_changed',
                    message: 'Email address changed since this link was issued',
                },
                410
            );
        case 'invalid':
...
auth.post('/matrix/exchange', async (c) => {
    ...
    if (!matrixToken) {
        return c.json(
            { code: 'matrix_token_missing', message: 'Missing Matrix access token' },
            401
        );
    }
    ...
    if (!whoami.ok) {
        ...
        return c.json(
            { code: 'matrix_token_invalid', message: 'Matrix access token is not valid' },
            401
        );
    }
...
auth.post('/token/refresh', async (c) => {
    ...
    switch (outcome.kind) {
        case 'ok': {
            ...
            if (!user) {
                return c.json(
                    { code: 'invalid_refresh_token', message: 'User no longer exists' },
                    401
                );
            }
            ...
            return c.json({
                token: access.token,
                refreshToken: outcome.rotated.token,
                userId: user.id,
            });
        }
        case 'reuse_detected': {
            ...
            return c.json(
                {
                    ...
                    message: 'Refresh token reuse detected; all sessions revoked. Re-authenticate.',
                },
                401
            );
        }
...
auth.post('/sessions/revoke', async (c) => {
    ...
    // Also invalidate access tokens already minted for this user, not just their
    // refresh tokens — otherwise a revoked session keeps working until its TTL.
    db.revokeUserTokensBefore(user.sub, 'admin_revoke');
...
auth.post('/password/change', async (c) => {
    ...
    // Force re-authentication on every other device: burn refresh tokens AND
    // invalidate access tokens issued before this password change.
    revokeAllForUser(record.id, 'password_change');
    db.revokeUserTokensBefore(record.id, 'password_change');
    return c.json({ ok: true });
});
...
auth.post('/password/reset/confirm', async (c) => {
    ...
    switch (outcome.kind) {
        ...
        case 'consumed':
            return c.json(
                { code: 'reset_token_consumed', message: 'Reset token already used' },
                410
            );
        case 'invalid':
        default:
...
auth.post('/account/delete/confirm', async (c) => {
    ...
    switch (outcome.kind) {
        ...
        case 'consumed':
            return c.json(
                { code: 'delete_token_consumed', message: 'Deletion token already used' },
                410
            );
        case 'user_mismatch':
            return c.json(
                {
                    code: 'delete_token_user_mismatch',
                    message: 'Deletion token does not belong to the authenticated user',
                },
                403
            );
        case 'invalid':
