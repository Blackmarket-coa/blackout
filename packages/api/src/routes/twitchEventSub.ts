...
import { jsonBodyLimit, WEBHOOK_MAX_BODY_BYTES } from '../middleware/body-limit';
...
const buildAlertContent = (event: NormalizedTwitchEvent): Record<string, unknown> => {
    ...
    switch (event.kind) {
        case 'follow':
            base.body = `[twitch] ${
                event.followerDisplayName ?? event.followerLogin
            } just followed.`;
            break;
        case 'subscribe':
            base.body = `[twitch] ${
                event.subscriberDisplayName ?? event.subscriberLogin
            } subscribed (tier ${event.tier})${event.isGift ? ' — gift' : ''}.`;
            break;
        case 'subscription_gift':
            base.body = `[twitch] ${event.gifterDisplayName ?? event.gifterLogin} gifted ${
                event.total
            } sub${event.total === 1 ? '' : 's'} (tier ${event.tier}).`;
            break;
        case 'cheer':
            base.body = `[twitch] ${
                event.cheererDisplayName ?? event.cheererLogin ?? 'Anonymous'
            } cheered ${event.bits} bits: ${event.message}`;
            break;
        case 'raid':
            base.body = `[twitch] ${
                event.fromChannelDisplayName ?? event.fromChannelLogin
            } raided with ${event.viewers} viewer${event.viewers === 1 ? '' : 's'}.`;
            break;
        case 'stream_online':
            base.body = `[twitch] ${
                event.broadcasterDisplayName ?? event.broadcasterLogin
            } went live.`;
            break;
        case 'stream_offline':
            base.body = `[twitch] ${
        ...
        case 'channel_points_redemption':
            base.body = `[twitch] ${event.userDisplayName ?? event.userLogin} redeemed "${
                event.rewardTitle
            }" (${event.rewardCost} points)${event.userInput ? `: ${event.userInput}` : ''}.`;
            break;
...
export const buildTwitchEventSubRoute = (options: EventSubRouteOptions = {}) => {
    ...
    // Cap the raw body BEFORE we buffer it for HMAC verification (M5). Twitch
    // EventSub deliveries are a few KB; 256 KiB is generous.
    router.use('*', jsonBodyLimit(WEBHOOK_MAX_BODY_BYTES));
    ...
    router.post('/', async (c) => {
        ...
        let parsed: {
            challenge?: string;
            subscription?: EventSubNotification['subscription'];
            event?: EventSubNotification['event'];
        };
        try {
        ...
        if (!normalized) {
            log.info('twitch_eventsub_unhandled_type', {
