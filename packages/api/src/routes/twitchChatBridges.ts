...
import { createBridge, deleteBridge, listBridgesForUser } from '../services/twitchChatBridge';
...
bridges.use('/', integrationsRateLimit);
bridges.use('/:id', integrationsRateLimit);
bridges.use('/:id/say', integrationsRateLimit);
...
bridges.post('/', async (c) => {
    ...
    try {
        ...
        log.error('twitch_chat_bridge_create_failed', {
